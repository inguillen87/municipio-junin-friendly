...
export function requireMunicipalPrefix(prefix,target=TARGET){
...
export function parseLinuxRoute(text,target=TARGET){
 const all=json(text);
 if(!Array.isArray(all)||all.length!==1||!all[0]||typeof all[0]!=='object')throw fault('ROUTE_OUTPUT_INVALID');
 const r=all[0],prefix=requireMunicipalPrefix(r.dst,target);
...
export function parseWindowsRoute(text,target=TARGET){
 const r=json(text);
 if(!r||typeof r!=='object'||Array.isArray(r))throw fault('ROUTE_OUTPUT_INVALID');
 const prefix=requireMunicipalPrefix(r.prefix,target);
...
export async function readMunicipalRoute({target=TARGET,platform=process.platform,run=execute,canAccess=access,systemRoot=process.env.SystemRoot}={}){
 try{
  requireMunicipalPrefix(MUNICIPAL_PREFIX,target);
  ...
  if(platform==='linux'){
   ...
   return parseLinuxRoute(stdout,target);
  }
  if(platform==='win32'){
   ...
   const {stdout}=await run(binary,['-NoLogo','-NoProfile','-NonInteractive','-Command',WINDOWS_ROUTE_COMMAND.replace(TARGET,target)],options);
   return parseWindowsRoute(stdout,target);
  }
