...
import {ensureCapture} from '../pm10/service.mjs';
import {beforeClockSession,nextCaptureFailure} from './capture-policy.mjs';
...
const fresh=c=>({schema:'clock-fleet-status.v1',clockId:c.clockId,serial:c.serial,status:'waiting',blocked:false,failureCount:0,connectionFailureCount:0,lastAttemptAt:null,lastCaptureAt:null,nextPollAt:null,lastError:null,uniqueLocalRecords:0,pendingLocalBatches:0,cloudReception:'not_configured'});
...
export async function captureClock(fleet,c,{collect=collectConfigured,credential=readCredential,route=readMunicipalRoute,now=()=>new Date(),signal}={}){
 ...
 try{
  ...
  preconnect=beforeClockSession(result);
...
function brief(s){return {clockId:s.clockId,status:s.status,blocked:s.blocked,lastAttemptAt:s.lastAttemptAt,lastCaptureAt:s.lastCaptureAt,lastError:s.lastError,connectionFailureCount:s.connectionFailureCount??0,uniqueLocalRecords:s.uniqueLocalRecords,nextPollAt:s.nextPollAt,cloudReception:'not_configured'};}
// Seed from verified per-clock files so one slow clock cannot disappear from the overview.
export async function seedFleetSummary(config){
 const results=new Map();
 for(const c of config.clocks){
  try{const saved=await boundState(path.join(config.stateDir,c.clockId),c);results.set(c.clockId,brief({...saved,status:c.enabled?saved.status:'disabled'}));}
  catch(e){results.set(c.clockId,{clockId:c.clockId,status:'review_required',lastError:safeCode(e),cloudReception:'not_configured'});}
 }
 return results;
}
export async function main(argv=process.argv.slice(2)){
 ...
 try{for(const [id,state] of await seedFleetSummary(config))results.set(id,state);await summary();await Promise.all(config.clocks.map(loop));}
