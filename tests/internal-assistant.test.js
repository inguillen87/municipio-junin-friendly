...
test('catálogo de ayuda expone las trece secciones reales y rutas existentes', () => {
  ...
  assert.deepEqual(catalog.sections.map((section) => section.label), [
    'Inicio', 'Personas', 'Estructura', 'Integración', 'Nómina', 'Comparar gestiones', 'Presupuesto', 'Asistente', 'Ausentismo', 'Licencias normativas', 'Calidad', 'Reportes', 'Ayuda',
  ]);
  ...
  assert.equal(catalog.glossary.length, 14);
});
...
test('ayuda general ofrece onboarding completo y GET anuncia las capacidades nuevas', async () => {
  ...
  assert.equal(help.payload.data.sections.length, 13);
  assert.equal(help.payload.relatedSections.length, 13);
  ...
  assert.equal(res.payload.guidance.sections.length, 13);
