...
test('la interfaz ART queda sujeta a capacidad, fuente real y límites explícitos', () => {
  ...
  assert.match(html, /Cruce automático de planillas ART/);
  assert.match(html, /data-art-galeno-file/);
  assert.match(html, /data-art-suss-file/);
  assert.match(html, /Excel y PDF reúnen el resumen/);
  assert.match(html, /Nombre de la exportación/);
  assert.match(html, /Datos actualizados hasta/);
  assert.match(html, /data-art-result hidden tabindex="-1"/);
  assert.match(html, /data-art-primary-downloads/);
  assert.match(html, /Más formatos para auditoría \(CSV\)/);
  ...
  assert.match(source, /supera el máximo de/);
  assert.match(source, /setAttribute\('aria-invalid', 'true'\)/);
  assert.match(source, /focusFirstInvalid\(invalidFields\)/);
  assert.match(source, /focusResult\(\)/);
  ...
  assert.match(build, /'assets\/payroll-art-noelia-adapter\.js'/);
