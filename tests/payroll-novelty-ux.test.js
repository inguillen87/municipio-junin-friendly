...
test('el workbench usa contrato gobernado y no inventa permisos ni persistencia local', () => {
  ...
  assert.match(source, /monthlyWriteAttempt\(\{url:API_URL\+\(native\?'\?version=2':''\),command:'prepare',payload:preparedDraft,key:preparedDraftKey/);
  ...
  assert.match(source, /payload:attempt\.payload,key:attempt\.idempotencyKey/);
  assert.match(source, /'Idempotency-Key':attempt\.key\},body:attempt\.body/);
  assert.match(source, /pendingWrite=null;pendingTransitionAttempts\.clear\(\)/);
  ...
  assert.match(source, /if \(changed\)[\s\S]{0,150}discardLocalDraft\(\)/);
  assert.match(source, /\(!canPrepare \|\| limitsChanged\) && !pendingWrite/);
  assert.match(source, /const canPrepare = hasCapability\('payroll\.novelty\.prepare'\)/);
