...
test('integración declarada en API, legajo y build',()=>{assert.match(fs.readFileSync('api/internal-attendance.js','utf8'),/resource === 'clock-workdays'/);assert.match(fs.readFileSync('internal-dashboard.html','utf8'),/summaryDownload.dataset.payrollSummaryDownload/);const s=fs.readFileSync('relojes-marcaciones.html','utf8');assert.match(s,/data-workday-release="workdays-v2"/);assert.doesNotMatch(s,/id="clockHeatmap"/);assert.match(fs.readFileSync('scripts/build-friendly.mjs','utf8'),/'assets\/workday-panel.js'/)});
