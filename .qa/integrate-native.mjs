...
const auditFiles={'scripts/audit-payroll-formula-inventory.mjs':'f46067485684ce600ed9add4b7b9d7bc1c5750ab1b20c8145071232076e3df87','tests/payroll-formula-audit.test.js':'4683250e9861dc287a2f9ed5b75ee0a16add8106fe76ac8cd19548fa5f3fc2a3'};
const additional=['scripts/build-friendly.mjs','.vercelignore','scripts/verify-workforce-operations-browser.mjs',...Object.keys(auditFiles)];
...
if(process.argv[2]==='apply'){
 ...
 assert.equal(git('hash-object','scripts/verify-workforce-operations-browser.mjs'),'654e903d0ead0d32d535d85b49883f5c97972b74');
 assert.equal(git('hash-object','scripts/inventory-payroll-formulas.mjs'),'234ae824c649b66242c1ed2b3be6db6307cd06ff');
 assert.equal(git('hash-object','assets/payroll-formula-linter.js'),'122d7e7a546cc7773faadf784689a3d9e06d2169');
 ...
 for(const [file,expected] of Object.entries(auditFiles)){assert.ok(!fs.existsSync(file),file);assert.equal(hash(fs.readFileSync(path.join(meta,file))),expected,file);fs.mkdirSync(path.dirname(file),{recursive:true});fs.copyFileSync(path.join(meta,file),file);}
 replaceOnce('scripts/verify-workforce-operations-browser.mjs',"await page.waitForURL('**/login.html?**');","await page.waitForURL(u=>u.origin===origin&&u.pathname==='/acceso');const deniedUrl=new URL(page.url());assert.equal(deniedUrl.searchParams.get('next'),'internal-dashboard.html#legajos');assert.deepEqual([...deniedUrl.searchParams.keys()],['next']);");
 ...
 execFileSync(process.execPath,[path.join(meta,'.qa/browser-readiness.mjs')],{stdio:'inherit'});
...
const mismatches=Object.entries({...manifest,...auditFiles}).map(([file,expected])=>({file,expected,actual:hash(fs.readFileSync(file))})).filter(x=>x.expected!==x.actual);
...
if(process.argv[2]==='candidate'){
 ...
 const commit=await api('git/commits',{tree:tree.sha,parents:[base],message:'feat(personas): candidato de alta nativa y auditoría estructural de fórmulas\n\nAlta propia de legajos, validaciones y recuperación del intento original. Regresión de Personas corregida para acceso canónico sin debilitar la comprobación de sesión. Auditor privado infijo/posfijo, dependencias, ciclos potenciales y rutinas no ejecutadas, con pruebas sintéticas. Suite completa y navegador validados sobre este árbol. No aplica migraciones, no actualiza master, no crea empleados reales, no liquida ni paga haberes. Requiere cierre de la verificación SQL y despliegue antes de declararlo productivo.'});
