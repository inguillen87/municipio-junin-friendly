...
import { verifyMonthlyBootstrap, verifyMonthlyBatch, verifyMonthlyEmployee }
  from '../assets/payroll-native-monthly-model.js';

export const PAYROLL_NOVELTY_CONTRACT_VERSION = 'payroll-novelty-batch.v1';
export const PAYROLL_NATIVE_NOVELTY_CONTRACT_VERSION = 'payroll-novelty-batch.v2';
...
const CONTRACT_UUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
const isContractUuid = value => typeof value === 'string' && CONTRACT_UUID.test(value)
...
// An explicit native request carries its original displayed legajo as a
// snapshot, but only the verified contract UUID selects its subject.
export function normalizeNativePayrollNoveltyDraft(input) {
  exactObject(input, DRAFT_KEYS, 'PAYROLL_NOVELTY_DRAFT_INVALID',
    'El borrador no coincide con el contrato de novedades');
  if (input.sourceMode !== 'individual' || input.payrollType !== 'monthly'
      || !Array.isArray(input.rows) || input.rows.length !== 1) {
    fail('PAYROLL_NOVELTY_PREPARE_INVALID', 422,
      'El alta propia admite una novedad individual del mes');
  }
  const row = input.rows[0];
  exactObject(row, new Set([...ROW_KEYS, 'contractId', 'identityToken']),
    'PAYROLL_NOVELTY_ROW_INVALID', 'La fila no coincide con el contrato de novedades');
  if (!isContractUuid(row.contractId)
      || typeof row.identityToken !== 'string' || !/^[a-f0-9]{64}$/.test(row.identityToken)) {
    fail('PAYROLL_NOVELTY_ROW_INVALID', 422,
      'Volvé a verificar el alta propia antes de preparar la novedad');
  }
  const { contractId, identityToken, ...legacyValues } = row;
  const normalized = normalizePayrollNoveltyDraft({ ...input, rows: [legacyValues] });
  return Object.freeze({ ...normalized,
    contractVersion: PAYROLL_NATIVE_NOVELTY_CONTRACT_VERSION,
    rows: Object.freeze([Object.freeze({ ...normalized.rows[0],
      contractId: contractId.toLowerCase(), identityToken })]),
  });
}
...
function mappedSqlError(error) {
  ...
  const definitions = [
    ...
    ['PAYROLL_NOVELTY_CONTRACT_NOT_FOUND', 404, 'El vínculo laboral seleccionado no está disponible'],
    ['PAYROLL_NOVELTY_NATIVE_ONLY', 422, 'Este circuito requiere un alta propia de MuniControl'],
    ['PAYROLL_NOVELTY_IDENTITY_CHANGED', 409, 'Cambió la identidad del vínculo. Revisá el alta; la novedad no se reasignó'],
    ['PAYROLL_NOVELTY_PERIOD_OUTSIDE_EMPLOYMENT', 422, 'El período informado no coincide con las fechas del vínculo laboral'],
    ['PAYROLL_NOVELTY_VERSION_UNSUPPORTED', 409, 'Actualizá la pantalla para consultar este lote'],
...
function commandHash(context, command, payload, version = PAYROLL_NOVELTY_CONTRACT_VERSION) {
...
function verifiedMonthlyEnvelope(envelope, verify) {
  assertEnvelopeFlags(envelope);
  try { verify(envelope); }
  catch { fail('PAYROLL_NOVELTY_CONTRACT_DRIFT', 503,
    'La respuesta de novedades no pudo verificarse.'); }
  return envelope;
}

function monthlyUuid(value, code = 'PAYROLL_NOVELTY_BATCH_ID_INVALID') {
  if (code === 'PAYROLL_NOVELTY_CONTRACT_ID_INVALID' ? !isContractUuid(value)
    : typeof value !== 'string' || !UUID.test(value)) {
    fail(code, 422, 'El identificador seleccionado no es válido');
  }
  return value.toLowerCase();
}

export async function getPayrollNoveltyBootstrapV2(sql, principal, session) {
  const context = identityContext(principal, session);
  const result = await facade(sql, 'payroll_novelty_bootstrap_v2',
    [JSON.stringify(context)], ['jsonb']);
  return verifiedMonthlyEnvelope(result, verifyMonthlyBootstrap);
}

export async function getPayrollNoveltyEmployeeV2(sql, principal, session, contractId) {
  const context = identityContext(principal, session);
  const id = monthlyUuid(contractId, 'PAYROLL_NOVELTY_CONTRACT_ID_INVALID');
  const result = await facade(sql, 'payroll_novelty_employee_v2',
    [JSON.stringify(context), id], ['jsonb', 'uuid']);
  return verifiedMonthlyEnvelope(result, value => verifyMonthlyEmployee(value, id));
}

export async function readPayrollNoveltyV2(sql, principal, session, batchId) {
  const context = identityContext(principal, session), id = monthlyUuid(batchId);
  const result = await facade(sql, 'payroll_novelty_detail_v2',
    [JSON.stringify(context), id], ['jsonb', 'uuid']);
  return verifiedMonthlyEnvelope(result, value => {
    verifyMonthlyBatch(value.data, { mode: 'detail' });
    if (value.data.id !== id) throw Error('Different batch');
  });
}

export async function prepareNativePayrollNovelty(sql, principal, session, rawDraft, idempotencyKey) {
  const context = identityContext(principal, session);
  const draft = normalizeNativePayrollNoveltyDraft(rawDraft);
  const key = normalizePayrollNoveltyIdempotencyKey(idempotencyKey);
  const hash = commandHash(context, 'prepare', draft, PAYROLL_NATIVE_NOVELTY_CONTRACT_VERSION);
  const result = await facade(sql, 'payroll_novelty_prepare_v2', [
    JSON.stringify(context), draft.sourceMode, draft.periodMonth, draft.payrollType,
    JSON.stringify(draft.rows), key, hash,
  ], ['jsonb', '', 'date', '', 'jsonb', 'uuid', '']);
  return verifiedMonthlyEnvelope(result, value => {
    if (typeof value.replayed !== 'boolean') throw Error('Invalid receipt');
    const batch = verifyMonthlyBatch(value.data, { mode: 'receipt' });
    if (batch.contractVersion !== PAYROLL_NATIVE_NOVELTY_CONTRACT_VERSION
        || batch.periodMonth !== draft.periodMonth || batch.status !== 'draft'
        || batch.version !== 1 || batch.rows[0].employmentContractId !== draft.rows[0].contractId
        || batch.rows[0].subject.identityToken !== draft.rows[0].identityToken
        || batch.rows[0].legajo !== draft.rows[0].legajo) throw Error('Receipt does not match request');
  });
}

export async function transitionPayrollNoveltyV2(sql, principal, session, command, rawTransition, idempotencyKey) {
  const context = identityContext(principal, session);
  const transition = normalizePayrollNoveltyTransition(command, rawTransition);
  const key = normalizePayrollNoveltyIdempotencyKey(idempotencyKey);
  const hash = commandHash(context, command, transition, PAYROLL_NATIVE_NOVELTY_CONTRACT_VERSION);
  const result = await facade(sql, 'payroll_novelty_transition_v2', [
    JSON.stringify(context), transition.batchId, command, transition.expectedVersion,
    transition.reasonCode, transition.reasonReference, key, hash,
  ], ['jsonb', 'uuid', '', 'integer', '', '', 'uuid', '']);
  return verifiedMonthlyEnvelope(result, value => {
    if (typeof value.replayed !== 'boolean') throw Error('Invalid receipt');
    const batch = verifyMonthlyBatch(value.data, { mode: 'receipt' });
    if (batch.id !== transition.batchId || batch.version !== transition.expectedVersion + 1
        || batch.status !== { submit: 'submitted', approve: 'approved', reject: 'rejected', cancel: 'cancelled' }[command]) {
      throw Error('Receipt does not match request');
    }
  });
}

export async function exportPayrollNoveltyV2(sql, principal, session, batchId) {
  const context = identityContext(principal, session), id = monthlyUuid(batchId);
  const result = await facade(sql, 'payroll_novelty_export_v2',
    [JSON.stringify(context), id], ['jsonb', 'uuid']);
  return verifiedMonthlyEnvelope(result, value => {
    const batch = verifyMonthlyBatch(value.data, { mode: 'export' });
    const version = batch.contractVersion === PAYROLL_NATIVE_NOVELTY_CONTRACT_VERSION ? 'v2' : 'v1';
    if (value.contractVersion !== 'payroll-novelty-export.' + version
        || value.approvalEffect !== 'export_only' || batch.id !== id) throw Error('Invalid export');
  });
}
