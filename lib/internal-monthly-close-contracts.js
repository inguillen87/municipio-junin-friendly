...
function reconcileReportedNetByRepartition(conceptSource, bankSource) {
  ...
  const comparisons = repartitions.map((repartitionCode) => {
    ...
    for (const code of issueCodes) {
      blockingIssues.push(Object.freeze({ code, repartitionCode }));
    }
    return Object.freeze({
      repartitionCode,
  ...
  return Object.freeze({
    basis: 'reported_earnings_plus_family_less_retentions_vs_reported_bank_net',
...
export function buildMonthlyClosePrecheck(sources) {
  ...
  const reconciliation = reconcileReportedNetByRepartition(
