...
function emailMfaCoordinates(options = {}) {
  const flowHash = hash(options.flowHash, 'flowHash');
  const deliveryAttemptId = uuid(options.deliveryAttemptId, 'deliveryAttemptId');
  if (!UUID_V4.test(deliveryAttemptId)) {
    fail('IDENTITY_PAYLOAD_INVALID', 422, 'deliveryAttemptId es invalido');
  }
  return { flowHash, deliveryAttemptId };
}

export async function prepareIdentityEmailMfa(sql, options = {}) {
  if (!sql || typeof sql.query !== 'function') throw new TypeError('conexion SQL requerida');
  const coordinates = emailMfaCoordinates(options);
  const codeHash = hash(options.codeHash, 'codeHash');
  const expiresAt = isoFuture(options.expiresAt, 'expiresAt');
  const expectedVersion = integer(options.expectedVersion, 'expectedVersion');
  const idempotencyKey = uuid(options.idempotencyKey, 'idempotencyKey');
  if (!UUID_V4.test(idempotencyKey)) {
    fail('IDENTITY_IDEMPOTENCY_REQUIRED', 428, 'Idempotency-Key UUIDv4 es obligatorio');
  }
  const result = await sql.query(`
    SELECT tenant_identity_prepare_email_mfa(
      $1, $2::uuid, $3, $4::timestamptz, $5, $6::uuid
    ) AS result
  `, [coordinates.flowHash, idempotencyKey, codeHash,
    expiresAt, expectedVersion, coordinates.deliveryAttemptId]);
  return rows(result)[0]?.result || {};
}

export async function markIdentityEmailMfaDelivery(sql, options = {}) {
  if (!sql || typeof sql.query !== 'function') throw new TypeError('conexion SQL requerida');
  const coordinates = emailMfaCoordinates(options);
  if (typeof options.accepted !== 'boolean') {
    fail('IDENTITY_PAYLOAD_INVALID', 422, 'accepted es invalido');
  }
  const result = await sql.query(`
    SELECT tenant_identity_mark_email_mfa_delivery($1, $2::uuid, $3) AS result
  `, [coordinates.flowHash, coordinates.deliveryAttemptId, options.accepted]);
  return rows(result)[0]?.result || {};
}

export async function completeIdentityEmailMfa(sql, options = {}) {
  if (!sql || typeof sql.query !== 'function') throw new TypeError('conexion SQL requerida');
  const coordinates = emailMfaCoordinates(options);
  const codeHash = hash(options.codeHash, 'codeHash');
  const sessionId = uuid(options.sessionId, 'sessionId');
  const sessionExpiresAt = isoFuture(options.sessionExpiresAt, 'sessionExpiresAt');
  const device = optionalString(options.device, 'device', 120) || 'Navegador';
  const ipApprox = optionalString(options.ipApprox, 'ipApprox', 64);
  const expectedVersion = integer(options.expectedVersion, 'expectedVersion');
  const idempotencyKey = uuid(options.idempotencyKey, 'idempotencyKey');
  if (!UUID_V4.test(idempotencyKey)) {
    fail('IDENTITY_IDEMPOTENCY_REQUIRED', 428, 'Idempotency-Key UUIDv4 es obligatorio');
  }
  const commandHash = createHash('sha256').update(stableJson({
    command: 'complete_email_mfa', flowHash: coordinates.flowHash,
    deliveryAttemptId: coordinates.deliveryAttemptId, codeHash,
    sessionId, sessionExpiresAt, device, ipApprox, expectedVersion,
  })).digest('hex');
  const result = await sql.query(`
    SELECT tenant_identity_complete_email_mfa(
      $1, $2::uuid, $3, $4::uuid, $5::timestamptz,
      $6, $7, $8, $9::uuid, $10
    ) AS result
  `, [coordinates.flowHash, idempotencyKey, codeHash,
    sessionId, sessionExpiresAt, device, ipApprox, expectedVersion,
    coordinates.deliveryAttemptId, commandHash]);
  return rows(result)[0]?.result || {};
}
  ...
  try {
    ...
    if (options.requireDataPlaneReady === true) {
      ...
      if (!/^[a-f0-9]{40}$/.test(expectedReleaseSha)
        ...
        return accessError(res, 503, 'IDENTITY_RELEASE_NOT_CERTIFIED', 'El contrato de datos activo no está certificado');
      }
    }
...
export function identityGatewayDatabaseError(error) {
  ...
  const mappings = [
    ...
    ['IDENTITY_RELEASE_NOT_CERTIFIED', 503, 'IDENTITY_RELEASE_NOT_CERTIFIED', 'El contrato de datos activo no está certificado'],
    ...
    ['IDENTITY_EMAIL_MFA_FACTOR_REQUIRED', 409, 'IDENTITY_EMAIL_MFA_NOT_CONFIGURED', 'No hay un correo de recuperacion habilitado'],
    ['IDENTITY_EMAIL_MFA_COOLDOWN', 429, 'IDENTITY_EMAIL_MFA_COOLDOWN', 'Espera antes de solicitar otro codigo'],
    ['IDENTITY_EMAIL_MFA_IDEMPOTENCY_REUSED', 409, 'IDENTITY_IDEMPOTENCY_REUSED', 'Idempotency-Key reutilizada'],
    ['IDENTITY_EMAIL_MFA_REPLAY', 409, 'IDENTITY_MFA_REPLAY', 'Codigo MFA ya utilizado'],
    ['IDENTITY_EMAIL_MFA_ATTEMPT_STALE', 401, 'IDENTITY_FLOW_INVALID_OR_EXPIRED', 'Flujo invalido o vencido'],
    ['IDENTITY_EMAIL_MFA_DELIVERY_STALE', 401, 'IDENTITY_FLOW_INVALID_OR_EXPIRED', 'Flujo invalido o vencido'],
    ['IDENTITY_EMAIL_MFA_CHALLENGE_INVALID', 401, 'IDENTITY_FLOW_INVALID_OR_EXPIRED', 'Flujo invalido o vencido'],
    ['IDENTITY_EMAIL_MFA_FACTOR_STALE', 401, 'IDENTITY_FLOW_INVALID_OR_EXPIRED', 'Flujo invalido o vencido'],
    ['IDENTITY_EMAIL_MFA_FLOW_INVALID', 401, 'IDENTITY_FLOW_INVALID_OR_EXPIRED', 'Flujo invalido o vencido'],
    ['IDENTITY_EMAIL_MFA_PREPARE_INVALID', 422, 'IDENTITY_PAYLOAD_INVALID', 'Los datos MFA email no son validos'],
    ['IDENTITY_EMAIL_MFA_COMPLETE_INVALID', 422, 'IDENTITY_PAYLOAD_INVALID', 'Los datos MFA email no son validos'],
    ['IDENTITY_EMAIL_MFA_DELIVERY_INVALID', 503, 'IDENTITY_EMAIL_MFA_DELIVERY_INVALID', 'No se pudo acreditar el envio del codigo'],
    ...
    ['SOURCE_ONBOARDING_POLICY_DRIFT', 503, 'IDENTITY_SOURCE_RELEASE_RECERTIFICATION_REQUIRED', 'El contrato de datos activo requiere una nueva certificación'],
  ...
  const mapped = new IdentityGatewayError(match[2], match[1], match[3]);
  if (match[0] === 'IDENTITY_EMAIL_MFA_COOLDOWN') mapped.retryAfter = 45;
  return mapped;
}
