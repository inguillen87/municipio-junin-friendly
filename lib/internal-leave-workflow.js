...
const CASE_COLUMNS = `
  ...
  action.source_binding_id::text AS "sourceBindingId",
...
function mixedSummaryColumns(nominalPredicate) {
  return `
    action.id::text AS id, action.case_number::text AS "caseNumber",
    action.tenant_id::text AS "tenantId",
    action.source_binding_id::text AS "sourceBindingId",
    action.case_type AS "caseType",
    CASE WHEN ${nominalPredicate} THEN action.beneficiary_contract_id::text END AS "beneficiaryContractId",
    action.company_id AS "companyId",
    CASE WHEN ${nominalPredicate} THEN action.organization_unit_source_id END AS "organizationUnitSourceId",
    CASE WHEN ${nominalPredicate} THEN action.sector_source_id END AS "sectorSourceId",
    action.status, action.confidentiality,
    action.policy_version_id AS "policyVersionId",
    action.payload - 'employeeNote' AS payload,
    action.evidence_status AS "evidenceStatus",
    action.version, action.updated_at AS "updatedAt",
    CASE WHEN ${nominalPredicate}
      THEN COALESCE(identity.full_name, employee.nombre, 'Nombre no informado') END AS "subjectDisplayName",
    CASE WHEN ${nominalPredicate} THEN contract.legacy_legajo END AS "subjectLegajo",
    CASE WHEN ${nominalPredicate}
      THEN COALESCE(employee.sector, 'Sector no informado') END AS "subjectSector",
    (${nominalPredicate}) AS "nominalProjection"
  `;
}

export async function getActionCase(sql, principal, caseId) {
  const id = requiredUuid(caseId, 'case_id');
  const result = await query(sql, `
    SELECT ${CASE_COLUMNS} ${CASE_FROM}
    WHERE action.id = $1::uuid
      AND action.tenant_id = $2::uuid
      AND action.source_binding_id = $3::uuid
    LIMIT 1
  `, [id, principal.tenantId, principal.sourceBindingId]);
  return result[0] || null;
}

async function getPayrollActionCase(sql, principal, caseId) {
  const id = requiredUuid(caseId, 'case_id');
  const result = await query(sql, `
    SELECT ${CASE_PAYROLL_SUMMARY_COLUMNS},
           action.company_id AS "companyId",
           action.created_at AS "createdAt",
           action.submitted_at AS "submittedAt",
           action.decided_at AS "decidedAt",
           action.cancelled_at AS "cancelledAt"
    FROM action_case action
    WHERE action.id = $1::uuid
      AND action.tenant_id = $2::uuid
      AND action.source_binding_id = $3::uuid
...
async function getActionCaseRouting(sql, principal, caseId) {
  const id = requiredUuid(caseId, 'case_id');
  const result = await query(sql, `
...
export async function updateLeaveDraft(sql, principal, caseId, rawPayload, expectedVersion, idempotencyKey, actionSession) {
  const current = await getActionCaseRouting(sql, principal, caseId);
...
function areaReadPredicate(principal, values) {
  ...
  const restricted = hasActionCapability(principal, C.RESTRICTED_READ)
    ? ''
    : " AND action.confidentiality = 'standard'";
  for (const scope of validAreaScopes(principal, C.AREA_READ)) {
...
function nominalReadPredicate(principal, values) {
  const branches = [];
  ...
  if (hasActionCapability(principal, C.ALL_MANAGE)
      || hasActionCapability(principal, C.ALL_READ)) {
    branches.push(hasActionCapability(principal, C.RESTRICTED_READ)
      ? 'TRUE'
      : "action.confidentiality = 'standard'");
  }
  return branches.length ? `(${branches.join(' OR ')})` : 'FALSE';
}

function payrollReadPredicate(principal) {
  return hasActionCapability(principal, C.PAYROLL_READ)
    ? "(action.status = 'approved' AND action.confidentiality = 'standard')"
    : 'FALSE';
}

function readPredicates(principal, values) {
  const nominal = nominalReadPredicate(principal, values);
  const payroll = payrollReadPredicate(principal);
  return {
    nominal,
    access: nominal === 'FALSE' ? payroll
      : (payroll === 'FALSE' ? nominal : `(${nominal} OR ${payroll})`),
  };
}
...
function listViewPredicate(principal, view, values) {
  ...
  if (view === 'mine') {
    if (!principal?.employmentContractId || !hasActionCapability(principal, C.SELF_READ)) {
      return { access: 'FALSE', nominal: 'FALSE' };
    }
    const predicate = `action.beneficiary_contract_id = ${addParameter(values, principal.employmentContractId)}::uuid`;
    return { access: predicate, nominal: predicate };
  }
  if (view === 'area') {
    const predicate = areaReadPredicate(principal, values);
    return { access: predicate, nominal: predicate };
  }
  return readPredicates(principal, values);
}
...
export async function listLeaveCases(sql, principal, options = {}) {
  ...
  const values = [caseType, principal.tenantId, principal.sourceBindingId];
  const access = listViewPredicate(principal, view, values);
  const filters = [
    'action.case_type = $1',
    'action.tenant_id = $2::uuid',
    'action.source_binding_id = $3::uuid',
    access.access,
  ];
  ...
  const records = await query(sql, `
    SELECT ${mixedSummaryColumns(access.nominal)}
    ${CASE_FROM}
    WHERE ${where}
  ...
  return {
    data: records.map((record) => serializeActionCaseSummary(
      record,
      principal,
      record.nominalProjection === false ? 'payroll' : 'nominal',
    )),
...
async function actionTimeline(sql, principal, caseId) {
  ...
  `, [caseId, principal.tenantId, principal.sourceBindingId]);
...
export async function readLeaveCase(sql, principal, caseId) {
  const routing = await getActionCaseRouting(sql, principal, caseId);
  const accessMode = routing ? actionReadAccessMode(principal, routing) : null;
  if (!routing || !accessMode) notFound();
  if (accessMode === 'payroll') {
    const payrollRecord = await getPayrollActionCase(sql, principal, routing.id);
    if (!payrollRecord || actionReadAccessMode(principal, payrollRecord) !== 'payroll') notFound();
    return {
      data: serializeActionCase(payrollRecord, principal, 'payroll'),
      timeline: [],
      allowedCommands: [],
    };
  }
  const record = await getActionCase(sql, principal, routing.id);
  if (!record || actionReadAccessMode(principal, record) !== 'nominal') notFound();
...
function createSubjectPredicate(principal, values) {
  ...
  if (hasActionCapability(principal, C.AREA_CREATE)) {
    for (const scope of validAreaScopes(principal, C.AREA_CREATE)) {
      if (scope.scopeLevel === 'company') {
        const company = addParameter(values, scope.companyId);
...
async function getPublishedGrhSource(sql, principal) {
  const sources = await query(sql, `
    SELECT source_cutoff AS cutoff
    FROM source_import_batch
    WHERE source_system = 'GRH' AND source_database = $1
      AND validation_state = 'published'
    ORDER BY source_cutoff DESC, recorded_at DESC, id DESC
    LIMIT 1
  `, [principal.sourceDatabase]);
...
export async function getActionBootstrap(sql, principal) {
  ...
  if (subjectAccess === 'FALSE') {
    return {
      ...
      source: await getPublishedGrhSource(sql, principal),
  ...
  const [contractRows, reasonRows, source] = await Promise.all([
    query(sql, `
      ...
      JOIN source_import_batch batch
       ...
       AND batch.source_database = ${addParameter(subjectValues, principal.sourceDatabase)}
      ...
      WHERE contract.status = 'active'
        ...
        AND contract.legacy_company_id = ${addParameter(subjectValues, principal.sourceCompanyId)}::bigint
        AND ${subjectAccess}
    ...
    `),
    getPublishedGrhSource(sql, principal),
  ]);
