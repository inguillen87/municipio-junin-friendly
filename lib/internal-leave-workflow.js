...
import { actionSourceContextFields, actionSourceIsHistorical } from './action-source-context.js';
...
export function serializeActionCase(record, principal, accessMode = 'nominal') {
  ...
  return {
    ...
    ...actionSourceContextFields(record, ActionCenterError),
...
export function allowedCommandsForCase(principal, record) {
  actionSourceContextFields(record, ActionCenterError);
  if (actionSourceIsHistorical(record)) return [];
...
export async function readLeaveCase(sql, identityPrincipal, caseId, actionSession) {
  ...
  return {
    ...
    allowedCommands: !actionSourceIsHistorical(record) && Array.isArray(envelope.allowedCommands)
