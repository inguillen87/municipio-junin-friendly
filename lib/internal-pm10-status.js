...
export async function getPm10Status(sql,principal,session){
 ...
 try{const r=await sql.query('SELECT public.attendance_pm10_status_v1($1::text,$2::uuid,$3::int,$4::text,$5::uuid,$6::uuid) AS result',[email,session.id,session.version,session.releaseSha,t.id,t.membershipId]);return assertPm10Status((Array.isArray(r)?r:r?.rows)?.[0]?.result);}
