...
export function summarizePreparte(raw, result, period) {
  ...
  for (const row of result.rows) {
    ...
    if (!groups.has(id)) groups.set(id, { id, legajo: row.legajo, name: row.personLabel, keys: new Set(), days: new Set(), reviewDays: new Set(), proofs: [], intervals: [], extraSeconds: 0, ordinarySeconds: 0, issues: new Set() });
  ...
  const rows = [...groups.values()].map(group => {
    ...
    return {
      ...
      ordinarySeconds: overlap ? null : group.ordinarySeconds,
      extraSeconds: overlap || !extraIntervals ? null : group.extraSeconds,
      evidenceHash: createHash('sha256').update(JSON.stringify([group.name,[...group.keys].sort(),group.proofs.sort(),[...group.issues]])).digest('hex'),
  ...
  return {
    ...
    evidenceHash: createHash('sha256').update(JSON.stringify([period,raw.site,raw.timezone,result.rules,
      raw.events.map(e=>[e.eventRef,e.streamKey,e.localTimestamp,e.code,e.identityState,e.legajo,e.personLabel,e.issues]).sort((a,b)=>a[0].localeCompare(b[0])),
