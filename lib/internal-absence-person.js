...
export async function internalAbsencePerson(sql,req,binding,snapshot,{readEvents}={}){
 ...
 const events=await readEvents(sql,{query:{resource:'absenceevents',contractId:query.contractId,from:query.from,to:query.to,page:String(query.page),limit:String(query.limit)}});
 ...
 FROM grh_effective_absences_v1 a WHERE a.company_id=$1::bigint AND a.legajo=$2 AND a.fecha>=$3::date AND a.fecha<=$4::date
