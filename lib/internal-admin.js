...
const COMMAND_PAYLOAD_KEYS = Object.freeze({
  ...
  request_existing_membership: Object.freeze(['tenantId', 'email', 'roleKey', 'reason']),
  request_membership_reactivation: Object.freeze(['membershipId', 'reason']),
  approve_membership_change: Object.freeze(['requestId', 'reason']),
  reject_membership_change: Object.freeze(['requestId', 'reason']),
  ...
  update_account_profile: Object.freeze(['tenantId', 'targetEmail', 'displayName', 'reason']),
});
...
const TENANT_MEMBERSHIP_GOVERNANCE_COMMANDS = new Set([
  'request_existing_membership', 'request_membership_reactivation',
  'approve_membership_change', 'reject_membership_change',
]);
const ACCOUNT_PROFILE_GOVERNANCE_COMMANDS = new Set(['update_account_profile']);
...
function postgresUuid(value, name) {
  const text = requiredString(value, name, 64).toLowerCase();
...
export function normalizeInternalAdminCommand(body, idempotencyKey) {
  ...
  if (command === 'create_tenant') {
  ...
  } else if (command === 'invite_user' || command === 'request_existing_membership') {
    ...
    if (!isDeliverableEmail(payload.email)) {
  ...
  } else if (ACCOUNT_PROFILE_GOVERNANCE_COMMANDS.has(command)) {
    payload.tenantId = uuid(payload.tenantId, 'tenantId');
    payload.targetEmail = requiredString(payload.targetEmail, 'targetEmail', 254).toLowerCase();
    if (!isDeliverableEmail(payload.targetEmail)) {
      fail('INTERNAL_ADMIN_PAYLOAD_INVALID', 422, 'targetEmail inválido');
    }
    payload.displayName = requiredString(payload.displayName, 'displayName', 160)
      .replace(/\s+/gu, ' ');
    if (payload.displayName.length < 3 || /[\u0000-\u001f\u007f<>]/u.test(payload.displayName)) {
      fail('INTERNAL_ADMIN_DISPLAY_NAME_INVALID', 422, 'displayName no es válido');
    }
    payload.reason = requiredString(payload.reason, 'reason', 500);
    if (payload.reason.length < 3) {
      fail('INTERNAL_ADMIN_REASON_INVALID', 422, 'reason debe tener al menos 3 caracteres');
  ...
  } else if (TENANT_MEMBERSHIP_GOVERNANCE_COMMANDS.has(command)) {
    payload.reason = requiredString(payload.reason, 'reason', 500);
    if (payload.reason.length < 3) {
      fail('INTERNAL_ADMIN_REASON_INVALID', 422, 'reason debe tener al menos 3 caracteres');
    }
    if (command === 'request_membership_reactivation') {
      payload.membershipId = uuid(payload.membershipId, 'membershipId');
    } else {
      payload.requestId = uuid(payload.requestId, 'requestId');
    }
    ...
    if (command === 'update_access') {
      ...
      payload.employmentContractId = postgresUuid(payload.employmentContractId, 'employmentContractId');
  ...
  if (VERSIONED_COMMANDS.has(command)) {
    const minimumVersion = ['request_platform_owner_grant', 'request_existing_membership'].includes(command) ? 0 : 1;
    ...
    if (command === 'request_existing_membership' && expectedVersion !== 0) {
      fail('INTERNAL_ADMIN_VERSION_REQUIRED', 428, 'expectedVersion debe ser 0 para una membresía nueva');
    }
...
export async function getInternalAdminView(sql, session, resource = 'bootstrap', limit = 100, options = {}) {
  ...
  if (!/^[a-f0-9]{40}$/.test(releaseSha)) {
    fail('INTERNAL_ADMIN_RELEASE_NOT_CERTIFIED', 503, 'El contrato de datos activo no está certificado');
  }
  ...
  const result = await sql.query(`
    SELECT tenant_iam_admin_view_v4($1, $2::uuid, $3, $4, $5, $6::uuid, $7) AS result
...
export async function applyInternalAdminCommand(sql, session, command, options = {}) {
  if (!sql || typeof sql.query !== 'function') throw new TypeError('conexion SQL requerida');
  const actor = managedPlatformSession(session);
  if (ACCOUNT_PROFILE_GOVERNANCE_COMMANDS.has(command.command)) {
    const releaseSha = requiredString(options.releaseSha, 'releaseSha', 40).toLowerCase();
    if (!/^[a-f0-9]{40}$/.test(releaseSha)) {
      fail('INTERNAL_ADMIN_RELEASE_NOT_CERTIFIED', 503, 'El contrato de datos activo no está certificado');
    }
    ...
    const governance = await sql.query(`
      SELECT account_profile_governance_apply_v1(
  ...
  if (TENANT_MEMBERSHIP_GOVERNANCE_COMMANDS.has(command.command)) {
    const releaseSha = requiredString(options.releaseSha, 'releaseSha', 40).toLowerCase();
    if (!/^[a-f0-9]{40}$/.test(releaseSha)) {
      fail('INTERNAL_ADMIN_RELEASE_NOT_CERTIFIED', 503, 'El contrato de datos activo no está certificado');
    }
    ...
    `, [actor.email, actor.id, actor.version, releaseSha, command.command,
      command.idempotencyKey, releaseBoundHash, command.expectedVersion,
      JSON.stringify(command.payload)]);
    return resultRows(membership)[0]?.result || {};
  }
...
export function internalAdminDatabaseError(error) {
  ...
  const mappings = [
    ...
    ['TENANT_MEMBERSHIP_ALREADY_EXISTS', 409, 'INTERNAL_ADMIN_MEMBERSHIP_EXISTS', 'La cuenta ya tiene una membresía en este gobierno'],
    ['tenant_membership_identity_uk', 409, 'INTERNAL_ADMIN_MEMBERSHIP_EXISTS', 'La cuenta ya tiene una membresía en este gobierno'],
    ['TENANT_MEMBERSHIP_REQUEST_PENDING', 409, 'INTERNAL_ADMIN_MEMBERSHIP_REQUEST_PENDING', 'Ya existe una solicitud pendiente para esta cuenta y gobierno'],
    ['tenant_membership_change_request_pending_uk', 409, 'INTERNAL_ADMIN_MEMBERSHIP_REQUEST_PENDING', 'Ya existe una solicitud pendiente para esta cuenta y gobierno'],
    ['tenant_iam_event_actor_idempotency_uk', 409, 'INTERNAL_ADMIN_IDEMPOTENCY_REUSED', 'Idempotency-Key reutilizada'],
    ['TENANT_MEMBERSHIP_NOT_SUSPENDED', 409, 'INTERNAL_ADMIN_MEMBERSHIP_NOT_SUSPENDED', 'La membresía no está suspendida'],
    ['TENANT_MEMBERSHIP_TARGET_MANAGED_REQUIRED', 409, 'INTERNAL_ADMIN_MANAGED_IDENTITY_REQUIRED', 'La cuenta debe completar activación administrada antes de asociarse'],
    ['TENANT_MEMBERSHIP_TARGET_MFA_REQUIRED', 409, 'INTERNAL_ADMIN_TARGET_MFA_REQUIRED', 'La cuenta debe tener MFA activo antes de asociarse'],
    ['TENANT_MEMBERSHIP_TENANT_NOT_READY', 409, 'INTERNAL_ADMIN_TENANT_NOT_READY', 'El gobierno y su plano de datos deben estar activos y certificados'],
    ['TENANT_MEMBERSHIP_OWNER_REQUIRED', 403, 'INTERNAL_ADMIN_OWNER_REQUIRED', 'La operación requiere una persona propietaria global activa'],
    ['TENANT_MEMBERSHIP_ACTOR_MFA_REQUIRED', 403, 'INTERNAL_ADMIN_OWNER_MFA_REQUIRED', 'La persona que opera debe tener MFA activo'],
    ['TENANT_MEMBERSHIP_MAKER_CHECKER_UNAVAILABLE', 409, 'INTERNAL_ADMIN_MAKER_CHECKER_UNAVAILABLE', 'Se requieren dos personas propietarias activas con MFA'],
    ['TENANT_MEMBERSHIP_MAKER_CHECKER_REQUIRED', 409, 'INTERNAL_ADMIN_MAKER_CHECKER_REQUIRED', 'Otra persona propietaria debe decidir la solicitud'],
    ['TENANT_MEMBERSHIP_STALE_AUTHORITY_REVIEW_REQUIRED', 409, 'INTERNAL_ADMIN_STALE_AUTHORITY_REVIEW_REQUIRED', 'La membresía conserva permisos o alcances anteriores que deben revisarse antes de reactivarla'],
    ['TENANT_MEMBERSHIP_REQUEST_NOT_PENDING', 409, 'INTERNAL_ADMIN_MEMBERSHIP_REQUEST_CLOSED', 'La solicitud ya fue decidida'],
    ['TENANT_MEMBERSHIP_REQUEST_NOT_FOUND', 404, 'INTERNAL_ADMIN_NOT_FOUND', 'Solicitud de membresía no encontrada'],
    ['TENANT_IAM_ROLE_INVALID', 422, 'INTERNAL_ADMIN_ROLE_INVALID', 'El rol operativo solicitado no es válido'],
    ['TENANT_MEMBERSHIP_COMMAND_INVALID', 422, 'INTERNAL_ADMIN_PAYLOAD_INVALID', 'La operación de membresía no es válida'],
    ['TENANT_MEMBERSHIP_ROLE_SOD_CONFLICT', 409, 'INTERNAL_ADMIN_SOD_CONFLICT', 'El perfil operativo viola segregación de funciones'],
    ...
    ['ACCOUNT_PROFILE_TENANT_NOT_ACTIVE', 409, 'INTERNAL_ADMIN_TENANT_INACTIVE', 'El gobierno objetivo no está activo'],
    ['ACCOUNT_PROFILE_TENANT_NOT_READY', 503, 'INTERNAL_ADMIN_RELEASE_NOT_CERTIFIED', 'El gobierno y su contrato de datos deben estar certificados'],
    ['ACCOUNT_PROFILE_TARGET_NOT_ACTIVE', 409, 'INTERNAL_ADMIN_ACCOUNT_INACTIVE', 'La cuenta objetivo debe estar activa'],
    ['ACCOUNT_PROFILE_MEMBERSHIP_NOT_ACTIVE', 409, 'INTERNAL_ADMIN_MEMBERSHIP_INACTIVE', 'La cuenta objetivo no tiene una membresía activa en el gobierno indicado'],
    ['ACCOUNT_PROFILE_VERSION_CONFLICT', 409, 'INTERNAL_ADMIN_VERSION_CONFLICT', 'La identidad cambió; actualizá y reintentá'],
    ['ACCOUNT_PROFILE_UNCHANGED', 409, 'INTERNAL_ADMIN_ACCOUNT_PROFILE_UNCHANGED', 'El nombre visible ya tiene ese valor'],
    ['ACCOUNT_PROFILE_IDEMPOTENCY_KEY_REUSED', 409, 'INTERNAL_ADMIN_IDEMPOTENCY_REUSED', 'Idempotency-Key reutilizada'],
    ['ACCOUNT_PROFILE_COMMAND_INVALID', 422, 'INTERNAL_ADMIN_PAYLOAD_INVALID', 'La corrección de cuenta no es válida'],
    ...
    ['TENANT_IAM_RELEASE_REQUIRED', 503, 'INTERNAL_ADMIN_RELEASE_NOT_CERTIFIED', 'El contrato de datos activo no está certificado'],
  ];
