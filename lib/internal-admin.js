...
export async function getInternalAdminView(sql, session, resource = 'bootstrap', limit = 100) {
  ...
  const result = await sql.query(
    'SELECT tenant_iam_admin_view_v2($1, $2::uuid, $3, $4, $5) AS result',
    [actor.email, actor.id, actor.version, normalizedResource, safeLimit],
  );
  return resultRows(result)[0]?.result || {};
}

export async function applyInternalAdminCommand(sql, session, command) {
  if (!sql || typeof sql.query !== 'function') throw new TypeError('conexion SQL requerida');
  const actor = managedPlatformSession(session);
  const result = await sql.query(`
