...
export function sourceRequestShape(req, path, write = false, allowWorkspace = false) {
  let url; try { url = new URL(req.url || '/', CLOCK_SOURCE_ORIGIN); } catch { sourceFail('CLOCK_SOURCE_QUERY_INVALID'); }
  const workspace=allowWorkspace===true && !write && req.method==='GET' && path==='/api/internal-clock-source' && url.searchParams.size===1 && url.searchParams.get('view')==='workspace';
  const queryOk=req.query===undefined || req.query && typeof req.query==='object' && !Array.isArray(req.query) && (workspace ? Object.keys(req.query).length===1 && req.query.view==='workspace' : Object.keys(req.query).length===0);
  if (url.origin !== CLOCK_SOURCE_ORIGIN || url.pathname !== path || url.search && !workspace || url.hash || !queryOk) sourceFail('CLOCK_SOURCE_QUERY_INVALID');
