...
} from './internal-certified-release.js';
...
export async function requireCompatibleInternalAccess(req, res, options = {}) {
  ...
  if (managedCookiePresent) {
    try {
      let expectedReleaseSha = options.expectedReleaseSha;
      if (options.requireDataPlaneReady === true && expectedReleaseSha === undefined) {
        expectedReleaseSha = resolveInternalCertifiedReleaseSha(env);
      }
      ...
      if (error instanceof InternalCertifiedReleaseError) {
        return sendAccessError(
          res,
          503,
          'IDENTITY_RELEASE_NOT_CERTIFIED',
          'La version activa no esta certificada',
        );
      }
