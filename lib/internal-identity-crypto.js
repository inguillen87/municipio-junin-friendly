...
export function identitySecrets(env = process.env) {
  ...
  return Object.freeze({ tokenPepper, sessionSecret, encryptionKey });
}
...
export function encryptMfaSecret(secret, encryptionKey, options = {}) {
  if (typeof secret !== 'string' || secret.length < 16 || secret.length > 256) {
    throw new TypeError('secreto MFA invalido');
  }
  ...
  cipher.setAAD(Buffer.from('municontrol:mfa:v1', 'utf8'));
...
export function decryptMfaSecret(value, encryptionKey) {
  try {
    ...
    decipher.setAAD(Buffer.from('municontrol:mfa:v1', 'utf8'));
