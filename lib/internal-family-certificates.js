...
export async function prepareSchoolCertificate(input, { validatePdf = validateSchoolCertificatePdf } = {}) {
  exact(input, PAYLOAD_KEYS, 'INVALID_PAYLOAD');
  if (!schoolCertificateContractId(input.contractId) || typeof input.familyId !== 'string' || !FAMILY.test(input.familyId)
  ...
  return { ...input, contractId: input.contractId.toLowerCase() };
}
...
async function facade(sql, statement, values) {
  try {
...
export function validateSchoolCertificateReport(data, contractId = null) {
  exact(data, ['version', 'rows', 'scope', 'canRegister', 'storage']);
  exact(data.scope, ['cohort', 'sourceCutoffFrom', 'sourceCutoffTo', 'currentCensusCertified', 'payrollEligibilityCertified']);
  ...
  if (data.version !== 'family-schooling.v1' || typeof data.canRegister !== 'boolean' || !Array.isArray(data.rows) || data.rows.length > 5000
  ...
  for (const row of data.rows) {
    exact(row, ['contractId', 'legajo', 'employeeName', 'familyId', 'familyName', 'birthDate', 'familyEndDate', 'identityToken', 'sourceCutoff', 'administrativeActive', 'certificate', 'historyCount']);
    const key = `${row.contractId}:${row.familyId}`;
    if (!schoolCertificateContractId(row.contractId) || contractId && row.contractId !== contractId || !text(row.legajo, 64)
        || !(row.employeeName === null || text(row.employeeName, 300)) || !(row.familyName === null || text(row.familyName, 300)) || typeof row.familyId !== 'string' || !FAMILY.test(row.familyId)
...
export async function readSchoolCertificates(sql, principal, session, contractId = null) {
  ...
  return validateSchoolCertificateReport(await facade(sql,
    'SELECT public.school_certificate_read_v1($1::text,$2::uuid,$3::integer,$4::text,$5::uuid,$6::uuid,$7::uuid) AS result',
    [...principalValues(principal, session), id]), id);
}
export async function registerSchoolCertificate(sql, principal, session, payload, key) {
  if (!schoolCertificateUuid(key)) schoolCertificateFail('IDEMPOTENCY_KEY_INVALID');
  const data = await facade(sql,
    'SELECT public.school_certificate_register_v1($1::text,$2::uuid,$3::integer,$4::text,$5::uuid,$6::uuid,$7::jsonb,$8::text) AS result',
    [...principalValues(principal, session), JSON.stringify(payload), key.toLowerCase()]);
  exact(data, ['version', 'certificateId', 'duplicate']);
  if (data.version !== 'family-schooling-register.v1' || !schoolCertificateUuid(data.certificateId) || typeof data.duplicate !== 'boolean') schoolCertificateFail('CONTRACT_DRIFT');
  return data;
}
export async function downloadSchoolCertificate(sql, principal, session, certificateId) {
  if (!schoolCertificateUuid(certificateId)) schoolCertificateFail('QUERY_INVALID');
  const data = await facade(sql,
    'SELECT public.school_certificate_download_v1($1::text,$2::uuid,$3::integer,$4::text,$5::uuid,$6::uuid,$7::uuid) AS result',
    [...principalValues(principal, session), certificateId.toLowerCase()]);
  exact(data, ['version', 'filename', 'contentBase64', 'sha256', 'byteLength']);
  if (data.version !== 'family-schooling-download.v1' || !filename(data.filename) || !count(data.byteLength) || data.byteLength < 1) schoolCertificateFail('CONTRACT_DRIFT');
