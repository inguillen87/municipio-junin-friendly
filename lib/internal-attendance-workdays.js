...
import {personContextOptions,workdayPersonReference,PERSON_CONTEXT_VERSION} from './workday-person-context.js';
import {normalizeReviewCause,filterReviewCause,summarizeReviewCauses} from '../assets/workday-review-causes.js';
...
export function normalizeContinuousWorkdayQuery(options={}) {
  ...
  let cause;try{cause=normalizeReviewCause(options.cause===undefined?'all':options.cause);}catch{fail('ATTENDANCE_WORKDAY_QUERY_INVALID',400,'Causa de revisión inválida');}
  let personOptions;try{personOptions=personContextOptions(options);}catch{fail('ATTENDANCE_PERSON_CONTEXT_INVALID',400,'Contexto de persona inválido');}
  return {...q,...personOptions,status,cause,includeReviewCauses:options.cause!==undefined,snapshot:q.snapshot?.toLowerCase() || null};
}
...
async function loadContinuousWorkdays(sql,principal,options,session) {
  ...
  if ((q.search || q.personRef) && !raw.nominalReadAllowed) fail('ATTENDANCE_CAPABILITY_REQUIRED',403,'Tu perfil no habilita búsquedas por persona');
  let result;
...
export async function getAttendanceWorkdaysV2(sql,principal,options,session) {
  ...
  const reference=row=>workdayPersonReference({tenantId:principal.tenant.id,siteKey:raw.site.key,revision:raw.revision},row);
  const contextRows=q.personRef?result.rows.filter(row=>reference(row)===q.personRef):result.rows;
  if(q.personRef&&(!contextRows.length||contextRows.some(row=>row.identityState!=='mapped')))fail('ATTENDANCE_PERSON_CONTEXT_CHANGED',409,'El vínculo seleccionado no está disponible en este corte. Volvé al listado.');
  const baseRows=filterContinuousWorkdays(contextRows,{search:q.search,status:q.status});
  const rows=filterReviewCause(baseRows,q.cause),total=rows.length;
  const page=rows.slice((q.page-1)*q.pageSize,q.page*q.pageSize).map(({personKey,key,...row})=>({...row,
    ...(q.includePersonContext?{personContextRef:raw.nominalReadAllowed&&row.identityState==='mapped'?reference({key}):null}:{}),
  ...
  if(q.includeReviewCauses){out.filters.cause=q.cause;out.reviewFacets=summarizeReviewCauses(baseRows);}
  if(q.includePersonContext){out.filters.personRef=q.personRef;out.personContext={version:PERSON_CONTEXT_VERSION,scope:'selected_site_device_contract',reference:q.personRef};}
  if (Buffer.byteLength(JSON.stringify(out))>2*1024*1024) fail('ATTENDANCE_WORKDAY_WINDOW_TOO_LARGE',413,'Reducí el tamaño de página para consultar el detalle');
  return out;
}
