...
import { reconstructWorkdays, reconstructContinuousWorkdays, CONTINUOUS_WORKDAY_RULES, filterWorkdays, summarizeWorkdays } from './attendance-workdays.js';
...
export function normalizeContinuousWorkdayQuery(options={}) {
  ...
  const q=normalizeWorkdayQuery(options);
  return {...q,snapshot:q.snapshot?.toLowerCase() || null};
}
...
export async function getAttendanceWorkdaysV2(sql,principal,options,session) {
  ...
  const out={version:'clock-workdays.v2',generatedAt:raw.generatedAt,sourceMode:'continuous',snapshotId:raw.revision,
    ...
    summary:summarizeWorkdays(rows),periodSummary:result.summary,rows:page,
