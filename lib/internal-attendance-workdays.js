...
export function normalizeContinuousWorkdayQuery(options={}) {
  ...
  return {...q,status,cause,includeReviewCauses:options.cause!==undefined,snapshot:q.snapshot?.toLowerCase() || null};
}
...
export async function getAttendanceWorkdaysV2(sql,principal,options,session) {
  ...
  const baseRows=filterContinuousWorkdays(result.rows,{search:q.search,status:q.status});
