...
const AREA_SCOPED_CAPABILITIES = new Set([
  C.AREA_CREATE,
  C.AREA_READ,
  C.AREA_UPDATE,
  C.AREA_SUBMIT,
  C.AREA_CANCEL_PENDING,
  C.AREA_DECIDE,
  C.AREA_CANCEL_APPROVED,
]);
...
export function publicPrincipal(principal) {
  const capabilities = [...principal.capabilities]
    ...
    .filter((capability) => !AREA_SCOPED_CAPABILITIES.has(capability)
      || principal.areaScopes.some((scope) => scope.capabilityKey === capability))
    .sort();
  return {
