...
const REASONS = Object.freeze({
  ...
  close: new Set(['closed_for_history']),
  annul: new Set(['annulled_by_authority']),
});
const STATUSES = new Set(['prepared', 'submitted', 'approved', 'closed', 'annulled', 'rejected', 'cancelled']);
const COMMANDS = new Set(['submit', 'approve', 'reject', 'cancel', 'close', 'annul']);
...
const REASON_CODES = new Set([
  ...
  'closed_for_history', 'annulled_by_authority',
...
function assertRun(run, { includeSources, includeAllowedCommands }) {
  ...
  if (!exactContractObject(run, expectedKeys)
      ...
      || run.closeApproved !== ['approved', 'closed'].includes(run.status)
