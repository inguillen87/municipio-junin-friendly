...
function mappedSqlError(error) {
  ...
  const definitions = [
    ...
    ['ATTENDANCE_RELEASE_NOT_CERTIFIED', 503, 'El contrato de datos activo no está certificado'],
...
function assertNoSensitiveResponse(value, path = 'response') {
  ...
  const visit = (node, current) => {
    ...
    for (const [key, child] of Object.entries(node)) {
      // The SQL connector projection exposes only the credential generation,
      // never its value or hash. Keep this exception exact, scoped and numeric.
      if (key === 'tokenVersion' && /^list\.connector\.\d+$/.test(current)
          && Number.isSafeInteger(child) && child > 0) continue;
...
export async function ingestAttendanceBatch(sql, normalized, tokenSha256, releaseSha) {
  ...
  if (!/^[a-f0-9]{40}$/.test(sha)) {
    fail('ATTENDANCE_RELEASE_NOT_CERTIFIED', 503, 'El contrato de datos activo no está certificado');
  }
