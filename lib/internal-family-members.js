...
function familyVersion(version) {
  if (![1, 2].includes(version)) throw employeeFamilySafeError({ code: 'SCHOOL_CERTIFICATE_QUERY_INVALID' });
}
export function validateEmployeeFamilyContext(data, contractId, version = 1) {
  familyVersion(version);
  exact(data, ['version', 'subject', 'canDeclare']);
  const native = version === 2 && data.subject?.origin === 'MUNICONTROL';
  exact(data.subject, ['contractId', 'legajo', 'employeeName', 'sourceCutoff', 'identityToken', ...(native ? ['origin', 'registrationId', 'registeredAt'] : [])]);
  const subject = data.subject;
  if (data.version !== `employee-family-context.v${version}` || typeof data.canDeclare !== 'boolean' || subject.contractId !== contractId || !schoolCertificateContractId(subject.contractId)
    ...
    || typeof subject.identityToken !== 'string' || !HEX.test(subject.identityToken)
    || (native ? subject.sourceCutoff !== null || !schoolCertificateUuid(subject.registrationId) || !timestamp(subject.registeredAt) || !subject.employeeName : !timestamp(subject.sourceCutoff))) employeeFamilyFail('CONTRACT_DRIFT');
  return data;
}
export function validateEmployeeFamilyDeclaration(data, version = 1, expected = null) {
  familyVersion(version);
  exact(data, ['version', 'familyRef', 'identityToken', 'state', 'recordedAt', 'duplicate', ...(version === 2 ? ['contractId', 'contractIdentityToken'] : [])]);
  exact(data.familyRef, ['kind', 'id']);
  if (data.version !== `employee-family-declare.v${version}` || data.familyRef.kind !== 'own' || !schoolCertificateUuid(data.familyRef.id)
  ...
  if (version === 2 && (!schoolCertificateContractId(data.contractId) || !HEX.test(data.contractIdentityToken ?? '')
    || expected && (data.contractId !== expected.contractId || data.contractIdentityToken !== expected.contractIdentityToken))) employeeFamilyFail('CONTRACT_DRIFT');
  return data;
}
...
export async function readEmployeeFamilyContext(sql, principal, session, contractId, { version = 1 } = {}) {
  familyVersion(version);
  ...
  return validateEmployeeFamilyContext(await execute(sql,
    `SELECT public.employee_family_context_v${version}($1::text,$2::uuid,$3::integer,$4::text,$5::uuid,$6::uuid,$7::uuid) AS result`,
    [...schoolCertificatePrincipalValues(principal, session), id]), id, version);
}
export async function declareEmployeeFamily(sql, principal, session, payload, key, { version = 1 } = {}) {
  familyVersion(version);
  ...
  return validateEmployeeFamilyDeclaration(await execute(sql,
    `SELECT public.employee_family_declare_v${version}($1::text,$2::uuid,$3::integer,$4::text,$5::uuid,$6::uuid,$7::jsonb,$8::text) AS result`,
    [...schoolCertificatePrincipalValues(principal, session), JSON.stringify(payload), key.toLowerCase()]), version, payload);
}
export async function readEmployeeFamilyAttempt(sql, principal, session, key) {
  if (!schoolCertificateUuid(key)) throw employeeFamilySafeError({ code: 'SCHOOL_CERTIFICATE_QUERY_INVALID' });
  const data = validateEmployeeFamilyDeclaration(await execute(sql,
    'SELECT public.employee_family_attempt_v2($1::text,$2::uuid,$3::integer,$4::text,$5::uuid,$6::uuid,$7::text) AS result',
    [...schoolCertificatePrincipalValues(principal, session), key.toLowerCase()]), 2);
  if (data.duplicate !== true) employeeFamilyFail('CONTRACT_DRIFT');
  return data;
}
