import { employeeDraft, EmployeeInputError, EMPLOYEE_JURISDICTION_CODES } from '../assets/native-employee-contract.js';
...
export function nativeEmployeeError(e){
 ...
 if(/\bNATIVE_EMPLOYMENT_CATALOG_BUSY\b/.test(e?.message||''))return new NativeEmployeeError('BUSY',...messages.BUSY);
...
export function validateReceipt(r){
 if(!r||r.version!=='native-employee.v1'||!uuid(r.contractId)||!uuid(r.registrationId)||!/^[1-9]\d{0,8}$/.test(r.legajo)||typeof r.name!=='string'||r.origin!=='MUNICONTROL'||r.accountCreated!==false||r.payrollCalculated!==false||typeof r.replayed!=='boolean'||(Object.hasOwn(r,'jurisdictionCode')&&!EMPLOYEE_JURISDICTION_CODES.includes(r.jurisdictionCode)))fail('CONTRACT_INVALID',503,'La confirmación del alta no pudo verificarse.');
 return r;
}
export async function employeeOperation(sql,principal,session,operation,input={}){
 const ctx=JSON.stringify(employeeContext(principal,session));let query,values,declaredJurisdiction;
 try{
  if(operation==='bootstrap') {query='SELECT public.native_employee_bootstrap_v1($1::jsonb) AS result';values=[ctx];}
   ...
   if(operation==='attempt'){query='SELECT public.native_employee_attempt_v1($1::jsonb,$2::uuid) AS result';values=[ctx,input.key];}
    ...
    const d=employeeDraft(input.body.draft);declaredJurisdiction=d.jurisdictionCode;
  ...
  if(operation!=='bootstrap'){
   const receipt=validateReceipt(r);
   if(operation==='create'&&receipt.jurisdictionCode!==declaredJurisdiction)fail('CONTRACT_INVALID',503,'La jurisdicción del alta confirmada no coincide con el formulario. Consultá el mismo intento.');
   return receipt;
  }
