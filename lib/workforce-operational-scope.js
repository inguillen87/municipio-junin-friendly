...
export function operationalDirectorySql(baseSql) {
  return `${baseSql}, closed_month AS (
    ...
    WHERE r.closure_status='closed'
      /* 002 validates each fact's date against its run; 096 preserves that
         equality while remapping runs. Bound the fact side too so the monthly
         date index is usable before reconstructing historical source rows. */
      AND f.payroll_date >= (SELECT month FROM closed_month)
      AND f.payroll_date < (SELECT month FROM closed_month) + interval '1 month'
