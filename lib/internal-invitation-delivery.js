...
function messageFor(input, expectedOrigin, route) {
  ...
  const subject = route.temporarySharedInbox
    ? `Activacion MuniControl | ${route.identityEmail} | ${route.roleLabel} | ${route.tenantLabel}`
    : `Invitacion para acceder a ${tenantName}`;
  ...
  const temporaryText = route.temporarySharedInbox ? [
    ...
    'El codigo activa solamente la identidad indicada; no comparte permisos ni sesiones.',
    '',
  ] : [];
  const text = [
    ...temporaryText,
  ...
  const temporaryHtml = route.temporarySharedInbox ? [
    '<div style="padding:14px;background:#fff8e7;border-left:4px solid #d39400;">',
    '<p style="margin:0 0 8px;font-weight:700;">Entrega temporal autorizada para la muestra</p>',
    `<p style="margin:0 0 4px;"><strong>Identidad de acceso:</strong> ${escapeHtml(route.identityLabel)} (${escapeHtml(route.identityEmail)})</p>`,
    `<p style="margin:0 0 4px;"><strong>Rol:</strong> ${escapeHtml(route.roleLabel)}</p>`,
    `<p style="margin:0 0 4px;"><strong>Ámbito:</strong> ${escapeHtml(route.tenantLabel)}</p>`,
    `<p style="margin:0 0 4px;"><strong>Buzón compartido:</strong> ${escapeHtml(route.maskedRecipient)}</p>`,
    `<p style="margin:0 0 4px;"><strong>Vigencia de la ruta:</strong> ${escapeHtml(temporaryValidity)}</p>`,
    '<p style="margin:8px 0 0;">El código activa solamente la identidad indicada; no comparte permisos ni sesiones.</p>',
    '</div>',
  ] : [];
  const html = [
  ...
  return Object.freeze({
    to: route.recipient, subject, text, html,
...
export function createResendInvitationDelivery(options = {}) {
  ...
  async function deliverInvitation(input) {
    const configuration = deliveryConfiguration(env, fetchImpl, options.now);
    recipient(input?.to);
    ...
    const message = messageFor(input, configuration.origin, route);
