...
import { canonicalClockOperationPointNames } from './attendance-point-naming.js';
...
export function assertClockOperationsResponse(result, query) {
  ...
  if (!result.collection || !['no_data','import_incomplete','operator_snapshot','continuous_receipts'].includes(result.collection.status)
...
export async function getAttendanceClockOperations(sql, principal, options, session) {
  ...
  try {
    ...
    const verified=assertClockOperationsResponse(rows?.[0]?.result,query);
    return assertClockOperationsResponse(canonicalClockOperationPointNames(verified,principal),query);
  } catch (error) {
