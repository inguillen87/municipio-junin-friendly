...
function subject(value) {
  exact(value, ['contractId', 'legajo', 'employeeName', 'identityToken', 'sourceCutoff']);
  if (!contract(value.contractId) || !fixedLegajo(value.legajo) || !(value.employeeName === null || text(value.employeeName, 1, 300))
    || !fixedHash(value.identityToken) || !stamp(value.sourceCutoff)) fixedFail('CONTRACT_DRIFT');
}
...
export function validateFixedResponse(data, resource, expected = {}) {
  if (resource === 'bootstrap') {
    ...
    if (data.version !== 'payroll-fixed-bootstrap.v1' || !['tenantId', 'membershipId', 'certifiedBindingId'].every(key => fixedUuid(data.principal[key]))
      ...
      || data.principal.capabilities.some(key => typeof key !== 'string' || !/^payroll\.novelty\.[a-z.]+$/.test(key))
    ...
    if (data.version !== 'payroll-fixed-employee.v1' || data.subject.legajo !== expected.legajo) fixedFail('CONTRACT_DRIFT');
...
export async function fixedCall(sql, principal, session, resource, args = {}) {
  ...
  const definitions = { bootstrap: [], employee: [['legajo', 'text']], list: [['periodMonth', 'date']], detail: [['recordId', 'uuid']],
    attempt: [['command', 'text'], ['key', 'uuid']], export: [['periodMonth', 'date'], ['snapshotToken', 'text']],
  ...
  try {
    const response = await sql.query(`SELECT public.payroll_fixed_${resource}_v1($1::jsonb${fields.map(([, type], i) => `,$${i + 2}::${type}`).join('')}) AS result`, values);
