...
function parseConfiguration(env, now) {
  ...
  const expiresAt = rfc3339Instant(parsed.expiresAt);
  ...
  const routes = parsed.routes.map((candidate) => {
    exactObject(candidate, ['identityEmail', 'context', 'identityLabel', 'roleLabel', 'tenantLabel']);
  ...
  return Object.freeze({ recipient, expiresAt: expiresAt.toISOString(), routes: Object.freeze(routes) });
}
...
export function resolveTemporaryEmailRoute(options = {}) {
  ...
  if (Date.parse(configured.expiresAt) <= now.getTime()) invalid('TEMPORARY_ROUTE_EXPIRED');
  return Object.freeze({
    recipient: configured.recipient,
    maskedRecipient: maskEmail(configured.recipient),
