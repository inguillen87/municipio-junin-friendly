...
import {canonicalFleetPointNames} from './attendance-point-naming.js';
...
export async function getClockFleet(sql,principal,session){
 ...
 try{
  ...
  const data=assertClockFleet((Array.isArray(r)?r:r?.rows)?.[0]?.result);
  return assertClockFleet(canonicalFleetPointNames(data,principal));
 }catch(e){
