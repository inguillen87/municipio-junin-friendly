...
function mappedSqlError(error) {
  ...
  const definitions = [
    ...
    ['TIME_SOURCE_RELEASE_NOT_CERTIFIED', 503, 'El contrato de datos activo no está certificado'],
...
function internalPrincipal(resolved, identityPrincipal) {
  ...
  const actorPersonId = resolved?.actorPersonId == null
    ? null : String(resolved.actorPersonId).trim().toLowerCase();
  if (!resolved || typeof resolved !== 'object'
      ...
      || !Object.hasOwn(resolved, 'actorPersonId')
      || !UUID.test(certifiedBindingId)
      || (actorPersonId !== null && !UUID.test(actorPersonId))
...
export async function getTimeSourceBootstrap(sql, identityPrincipal, session) {
  ...
  const hasEmploymentLink = principal.actorPersonId !== null;
  return {
    principal,
    contract: TIME_SOURCE_REGISTRY_CONTRACT,
    feature: {
      governanceStatus: 'metadata_intake',
      canPropose: hasEmploymentLink && principal.capabilities.has('time.source.propose'),
      canApprove: hasEmploymentLink && principal.capabilities.has('time.source.approve'),
    ...
    allowedCommands: hasEmploymentLink
      ? allowed.filter((command) => command === 'create_draft') : [],
  };
}
...
export async function applyTimeSourceCommand(
  ...
  if (principal.actorPersonId === null) {
    fail('TIME_SOURCE_EMPLOYMENT_REQUIRED', 403,
      'La operación exige un vínculo laboral vigente para identificar al actor');
  }
  const coordinates = identityCoordinates(identityPrincipal, session);
