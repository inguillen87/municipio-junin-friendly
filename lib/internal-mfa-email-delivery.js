...
} from './internal-temporary-email-routing.js';
...
function deliveryConfiguration(env, fetchImpl, now) {
  ...
  try {
    assertTemporaryEmailRoutingConfiguration({ env, now });
  } catch (error) {
    if (error instanceof TemporaryEmailRoutingError) unavailable(error.reason);
    throw error;
  }
...
function temporaryRouteValidity(route) {
  if (route.temporaryRouteExpiryMode === 'manual'
      && route.temporaryRouteExpiresAt === null) {
    return 'Hasta desactivación manual del responsable de plataforma';
  }
  if (route.temporaryRouteExpiryMode === 'scheduled'
      && typeof route.temporaryRouteExpiresAt === 'string') {
    return route.temporaryRouteExpiresAt;
  }
  unavailable('CONFIGURATION_INVALID');
  return undefined;
}

function messageFor(input, currentTime, route) {
  if (!input || typeof input !== 'object' || Array.isArray(input)
      || Object.keys(input).some((key) => ![
        'to', 'code', 'expiresAt', 'contextLabel', 'idempotencyKey', 'identityEmail', 'tenantId',
      ].includes(key))) {
    unavailable('PAYLOAD_INVALID');
  }
  recipient(input.to);
  ...
  const subject = route.temporarySharedInbox
    ? `Codigo MuniControl | ${route.identityEmail} | ${route.roleLabel} | ${route.tenantLabel}`
    : 'Tu código de acceso seguro | MuniControl Junín';
  const temporaryValidity = route.temporarySharedInbox
    ? temporaryRouteValidity(route)
    : null;
  const temporaryText = route.temporarySharedInbox ? [
    'ENTREGA TEMPORAL AUTORIZADA PARA LA MUESTRA',
    `Identidad de acceso: ${route.identityLabel} (${route.identityEmail})`,
    `Rol: ${route.roleLabel}`,
    `Ambito: ${route.tenantLabel}`,
    `Buzon compartido: ${route.maskedRecipient}`,
    `Vigencia de la ruta compartida: ${temporaryValidity}`,
    'El codigo inicia solamente la identidad indicada; no comparte permisos ni sesiones.',
    '',
  ] : [];
  const text = [
    'MuniControl | Plataforma de gestión municipal de Junín',
    '',
    ...temporaryText,
  ...
  const temporaryHtml = route.temporarySharedInbox ? [
    '<div style="margin:0 0 22px;padding:16px;background:#fff8e7;border-left:4px solid #d39400;color:#5b4500;font-size:14px;line-height:1.55;">',
    '<strong>Entrega temporal autorizada para la muestra</strong><br>',
    `Identidad de acceso: ${escapeHtml(route.identityLabel)} (${escapeHtml(route.identityEmail)})<br>`,
    `Rol: ${escapeHtml(route.roleLabel)}<br>`,
    `Ámbito: ${escapeHtml(route.tenantLabel)}<br>`,
    `Buzón compartido: ${escapeHtml(route.maskedRecipient)}<br>`,
    `Vigencia de la ruta: ${escapeHtml(temporaryValidity)}<br>`,
    'El código inicia solamente la identidad indicada; no comparte permisos ni sesiones.',
    '</div>',
  ] : [];
  const html = [
    '<!doctype html>',
    ...
    '<p style="margin:0;font-size:12px;line-height:1.5;letter-spacing:1px;text-transform:uppercase;color:#dce8ec;">Plataforma de gestión municipal · Junín, Mendoza</p>',
    '</td></tr>',
    ...
    '</td></tr>',
    '<tr><td style="padding:18px 28px;background:#f6f8f9;border-top:1px solid #d8e1e5;font-size:12px;line-height:1.5;color:#607780;">Mensaje transaccional de seguridad · Plataforma MuniControl</td></tr>',
    '</table>',
    '</td></tr></table>',
    '</body></html>',
  ].join('');
  return Object.freeze({ to: route.recipient, subject, text, html, idempotencyKey });
}
...
export function createResendMfaEmailDelivery(options = {}) {
  ...
  function isConfigured() {
    try {
      deliveryConfiguration(env, fetchImpl, options.now);
      return true;
    } catch {
      return false;
    }
  ...
  async function deliver(input) {
    const currentTime = nowDate(options.now);
    const configuration = deliveryConfiguration(env, fetchImpl, currentTime);
    recipient(input?.to);
    recipient(input?.identityEmail ?? input?.to);
    let route;
    try {
      route = resolveTemporaryEmailRoute({
        env,
        now: currentTime,
        logicalRecipient: input?.to,
        identityEmail: input?.identityEmail ?? input?.to,
    ...
    const message = messageFor(input, currentTime, route);
    ...
    try {
      ...
      return Object.freeze({
        ...
        ...(route.temporarySharedInbox ? {
          maskedDestination: route.maskedRecipient,
          temporarySharedInbox: true,
          temporaryRouteExpiresAt: route.temporaryRouteExpiresAt,
          temporaryRouteExpiryMode: route.temporaryRouteExpiryMode,
        } : {}),
      });
    } catch (error) {
