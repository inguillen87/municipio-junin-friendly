...
export function reconstructContinuousWorkdays(input) {
  ...
  for (const [deviceKey, device] of devices) {
    ...
    const result = reconstructWorkdays({...input, events:adapted, model:device.model});
  ...
  return {version:CONTINUOUS_WORKDAY_RULES.version, rows, summary:summarizeWorkdays(rows),
