/** Exact owner-preauthorized private delivery; never arbitrary imports or document emission. */
import { getActionCenterSql } from './internal-actions.js';
import { fetchAuthorizedCipher,publicDeliveryReceipt,unpackAuthorizedPayload } from '../lib/payroll-private-transfer.js';
const MAX=1500000;
...
export function createPayrollSourceDeliveryHandler({getSql=getActionCenterSql,env=process.env,fetchImpl=fetch}={}){return async(req,res)=>{
 ...
 try{
  ...
  const keys=Object.keys(body).sort().join(',');let sql,cipher;let payloadMode=false;
  if(keys==='jobId,mode'&&body.mode==='pull'){
   ...
   if(descriptor.transport==='https_gzip'){cipher=unpackAuthorizedPayload(descriptor,cipher);payloadMode=true;}
  ...
  const [row]=await sql.query(payloadMode?'SELECT payroll_detail_payload_delivery_v1($1::uuid,$2::bytea) AS result':'SELECT payroll_detail_delivery_v1($1::uuid,$2::bytea) AS result',[body.jobId,'\\x'+cipher.toString('hex')]);
