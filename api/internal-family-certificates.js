...
function checkLength(req) {
  const length = header(req, 'content-length');
  if (length && !/^(?:0|[1-9][0-9]*)$/.test(length)) schoolCertificateFail('BODY_INVALID');
  if (length && Number(length) > SCHOOL_CERTIFICATE_MAX_BODY_BYTES) schoolCertificateFail('BODY_TOO_LARGE');
}
function parseJson(value) {
  // The payload is flat. Scan string tokens to reject repeated (including
  // escaped) property names before JSON.parse can silently overwrite them.
  const keys = new Set();
  try {
...
async function readBody(req) {
  ...
  if (value === undefined && typeof req[Symbol.asyncIterator] === 'function') {
    ...
    for await (const chunk of req) {
      ...
      size += bytes.length;
      if (size > SCHOOL_CERTIFICATE_MAX_BODY_BYTES) schoolCertificateFail('BODY_TOO_LARGE');
      chunks.push(bytes);
    }
    value = Buffer.concat(chunks, size);
  }
  if (Buffer.isBuffer(value)) {
    if (value.length > SCHOOL_CERTIFICATE_MAX_BODY_BYTES) schoolCertificateFail('BODY_TOO_LARGE');
    value = value.toString('utf8');
  }
  if (typeof value === 'string') {
    if (Buffer.byteLength(value) > SCHOOL_CERTIFICATE_MAX_BODY_BYTES) schoolCertificateFail('BODY_TOO_LARGE');
  ...
  return async function internalFamilyCertificates(req, res) {
    ...
    try {
      ...
      if (method === 'POST') {
        ...
        const payload = await prepareSchoolCertificate(await readBody(req), { validatePdf: dependencies.validatePdf });
        const sql = await sqlFn(env);
        const data = await registerSchoolCertificate(sql, access.principal, session, payload, key);
      ...
      if (q.resource === 'download') {
        const result = await downloadSchoolCertificate(sql, access.principal, session, q.certificateId);
        res.setHeader('Content-Type', 'application/pdf');
        res.setHeader('Content-Disposition', `attachment; filename="${result.filename}"`);
        res.setHeader('Content-Length', String(result.bytes.length));
        return res.status(200).end(result.bytes);
      }
      const data = await readSchoolCertificates(sql, access.principal, session, q.contractId ?? null);
