...
function query(req, method) {
  ...
  if (Object.hasOwn(values, 'version')) {
    if (!allowed.length || !(resource === 'report' || resource === 'family' ? ['2', '3', '4', '5'] : ['2', '3']).includes(values.version)) schoolCertificateFail('QUERY_INVALID');
...
export function createInternalFamilyCertificatesHandler(dependencies = {}) {
  ...
  return async function internalFamilyCertificates(req, res) {
    ...
    try {
      ...
      const version = q.version === '5' ? 5 : q.version === '4' ? 4 : q.version === '3' ? 3 : q.version === '2' ? 2 : 1;
      if (method === 'POST') {
