...
export function createPm10Receiver({env=process.env,getSql=getActionCenterSql,receive=receivePm10}={}){return async(req,res)=>{
 ...
 }catch(e){if(e instanceof AttendanceGatewayError)return send(res,e.status||400,{ok:false,code:e.code,error:e.message});return send(res,503,{ok:false,code:'PM10_NOT_READY',error:'Recepción no disponible; el colector debe conservar el lote'});}
};}
