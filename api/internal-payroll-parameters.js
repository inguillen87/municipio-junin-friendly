...
import { PARAMETER_MAX_BODY_BYTES, PayrollParameterError, readPayrollParameters, writePayrollParameters } from '../lib/internal-payroll-parameters.js';
...
function query(req) {
  ...
  const resource = q.resource || 'bootstrap'; delete q.resource;
  if (resource === 'list') { q.status ??= 'all'; q.page ??= '1'; q.limit ??= '20'; q.period ??= ''; }
  return { resource, input: q };
}
export function createInternalPayrollParametersHandler(dependencies = {}) {
  ...
  const read = dependencies.read ?? readPayrollParameters, write = dependencies.write ?? writePayrollParameters;
  return async (req, res) => {
    ...
    try {
      ...
      if (method === 'POST' && (!['prepare', 'submit', 'approve', 'reject', 'cancel'].includes(input.command) || Object.keys(req.query || {}).length)) error('PAYROLL_PARAMETER_COMMAND_INVALID', 400, 'Comando no admitido.');
      ...
      return res.status(method === 'POST' && input.command === 'prepare' ? 201 : 200).json({ ok: true, ...result });
    } catch (e) {
...
export default createInternalPayrollParametersHandler();
