...
export async function buildLegalRegistry(root,output){
 ...
 const result=await build({absWorkingDir:root,entryPoints:{'legal-registry':source},outdir:path.join(output,'assets/islands'),entryNames:'[name]-[hash]',
  bundle:true,minify:true,jsx:'automatic',format:'esm',platform:'browser',target:['es2020'],sourcemap:false,legalComments:'linked',metafile:true,logLevel:'warning',define:{'process.env.NODE_ENV':'"production"'}});
