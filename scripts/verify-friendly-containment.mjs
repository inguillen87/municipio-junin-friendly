...
for (const migration of [
  ...
  '016-email-mfa-retry-idempotency.sql',
]) {
  assert.match(
    ignore,
...
assert.equal(friendlyData.source.snapshotAt, '2026-09-10T15:17:30');
assert.equal(friendlyData.source.sha256.toLowerCase(), '5a604acfe5ea32832b630d8aab29e494038d4c8940b231e283a53d14112665c7');
assert.equal(friendlyData.workforce.historicalRecords, 2452);
assert.equal(friendlyData.workforce.active, 875);
assert.equal(friendlyData.workforce.inactive, 1577);
assert.equal(friendlyData.management.current.balance, 42);
assert.equal(friendlyData.management.previous.balance, 31);
assert.equal(friendlyData.absence.totalEvents, 31702);
