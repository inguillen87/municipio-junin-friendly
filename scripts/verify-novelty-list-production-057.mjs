...
if(!published)throw Error('NOVELTY_057_NOT_PUBLISHED');
...
if(![401,403].includes(r.status))throw Error('NOVELTY_057_ANONYMOUS_NOT_DENIED');
if(!/no-store/.test(r.headers.get('cache-control')||''))throw Error('NOVELTY_057_PRIVATE_RESPONSE_CACHE');
fs.mkdirSync('verification',{recursive:true});fs.writeFileSync('verification/novelty-list-057-production.json',JSON.stringify({commit,checkedAt:new Date().toISOString(),publishedAssetsMatch:true,expected,anonymousStatus:r.status,cacheControl:r.headers.get('cache-control'),realMunicipalSessionTested:false,productionApiWrites:0},null,2));
