...
try{
 ...
 canRead=false;const before=requests.length;await page.reload({waitUntil:'networkidle'});await page.locator('#organizationRows tr').waitFor();assert.equal(await page.locator('[data-sp-open]').count(),0);assert.equal(requests.length,before);checks.push('Sin permiso nominal no se ofrecen acciones ni se consulta el directorio.');
