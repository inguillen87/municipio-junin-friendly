...
const out=path.join(workspace,'verification/workday-review');fs.mkdirSync(out,{recursive:true});
const workdayAssets=['assets/workday-panel.js','assets/workday-panel-model.js','assets/workday-panel.css','assets/workday-export.js'];
...
try{
 ...
 assert.ok(requests.filter(r=>r.query.get('pageSize')==='100').every(r=>r.query.get('status')==='extra_open'&&r.query.get('snapshot')===REVIEW_CUT));
 ...
 const report={checksPassed:checks.length,checks,errors,syntheticApi:true,syntheticTiles:true,municipalSessionTested:false,productionTested:false,backendWrites:false,base,
