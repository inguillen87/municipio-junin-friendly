...
const patches=[
  ['</head>', '<link rel="stylesheet" href="/assets/clock-fleet-panel.css"><link rel="stylesheet" href="/assets/clock-fleet-workspace.css"><script type="module" src="/assets/clock-fleet-panel.js"></script><script type="module" src="/assets/clock-source-panel.js"></script></head>'],
  [
    "<section id=\"pm10Reception\"",
    "<section id=\"clockFleetReception\" data-clock-workspace=\"v1\" hidden aria-label=\"Central de recepción por equipo\"></section>\n<details class=\"ck-infrastructure\"><summary>Archivo original · consulta técnica independiente</summary><section id=\"clockSourceArchive\" hidden aria-label=\"Archivo original de relojes\"></section></details>\n<section id=\"pm10Reception\""
  ],
  [
...
export function buildClockFleet(root,output){for(const asset of ['attendance-point-label.js','clock-fleet-model.js','clock-fleet-panel.js','clock-fleet-panel.css','clock-source-model.js','clock-source-panel.js','clock-fleet-workspace-model.js','clock-fleet-workspace-panel.js','clock-fleet-workspace.css'])fs.copyFileSync(path.join(root,'assets',asset),path.join(output,'assets',asset));const file=path.join(output,'relojes-marcaciones.html');fs.writeFileSync(file,patchClockFleetHtml(fs.readFileSync(file,'utf8')));}
