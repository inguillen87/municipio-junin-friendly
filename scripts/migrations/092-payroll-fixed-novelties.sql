...
CREATE OR REPLACE FUNCTION public.payroll_fixed_immutable_v1() RETURNS trigger
...
CREATE TRIGGER payroll_fixed_novelty_immutable BEFORE UPDATE OR DELETE OR TRUNCATE ON public.payroll_fixed_novelty FOR EACH STATEMENT EXECUTE FUNCTION public.payroll_fixed_immutable_v1();
DROP TRIGGER IF EXISTS payroll_fixed_novelty_event_immutable ON public.payroll_fixed_novelty_event;
CREATE TRIGGER payroll_fixed_novelty_event_immutable BEFORE UPDATE OR DELETE OR TRUNCATE ON public.payroll_fixed_novelty_event FOR EACH STATEMENT EXECUTE FUNCTION public.payroll_fixed_immutable_v1();

CREATE OR REPLACE FUNCTION public.payroll_fixed_context_v1(p_context jsonb,capability text DEFAULT 'payroll.novelty.read') RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$
DECLARE ctx jsonb;
BEGIN
 ctx:=public.payroll_novelty_assert_context_v1(p_context,capability);
 ...
 IF capability IN ('payroll.novelty.prepare','payroll.novelty.approve') AND (ctx->>'actorPersonId' IS NULL OR ctx->>'employmentLinked' IS DISTINCT FROM 'true') THEN RAISE EXCEPTION 'PAYROLL_FIXED_EMPLOYMENT_REQUIRED'; END IF;
...
EXCEPTION WHEN OTHERS THEN
 ...
 RAISE;
END $$;
CREATE OR REPLACE FUNCTION public.payroll_fixed_lock_v1(ctx jsonb) RETURNS void
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$
BEGIN
...
END $$;
CREATE OR REPLACE FUNCTION public.payroll_fixed_capacity_v1() RETURNS void
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$
...
END $$;
CREATE OR REPLACE FUNCTION public.payroll_fixed_effects_v1() RETURNS jsonb
...
$$;
CREATE OR REPLACE FUNCTION public.payroll_fixed_date_v1(value text,required boolean DEFAULT true) RETURNS date
...
CREATE OR REPLACE FUNCTION public.payroll_fixed_text_v1(value text,min_length integer,max_length integer) RETURNS boolean
LANGUAGE sql IMMUTABLE SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$
...
CREATE OR REPLACE FUNCTION public.payroll_fixed_subject_v1(ctx jsonb,legajo text,hold_lock boolean DEFAULT false) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$
...
CREATE OR REPLACE FUNCTION public.payroll_fixed_identity_current_v1(ctx jsonb,item public.payroll_fixed_novelty) RETURNS boolean
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$
DECLARE fresh jsonb;
BEGIN
 fresh:=public.payroll_fixed_subject_v1(ctx,item.subject->>'legajo');
...
CREATE OR REPLACE FUNCTION public.payroll_fixed_proposal_json_v1(ctx jsonb,item public.payroll_fixed_novelty_event,identity_current boolean) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$
...
BEGIN
 ...
 can_review:=review_json IS NULL AND identity_current AND ctx->'capabilities' ? 'payroll.novelty.approve' AND ctx->>'employmentLinked'='true'
...
END $$;
CREATE OR REPLACE FUNCTION public.payroll_fixed_record_json_v1(ctx jsonb,item public.payroll_fixed_novelty) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$
DECLARE current_identity boolean; head_version integer; latest jsonb; approved jsonb; pending jsonb;
BEGIN
 current_identity:=public.payroll_fixed_identity_current_v1(ctx,item);
 SELECT max(e.version) INTO head_version FROM public.payroll_fixed_novelty_event e WHERE e.record_id=item.id;
 SELECT public.payroll_fixed_proposal_json_v1(ctx,e,current_identity) INTO latest FROM public.payroll_fixed_novelty_event e WHERE e.record_id=item.id AND e.command='propose' ORDER BY e.version DESC LIMIT 1;
 SELECT public.payroll_fixed_proposal_json_v1(ctx,p,current_identity) INTO approved FROM public.payroll_fixed_novelty_event p
 ...
 RETURN jsonb_build_object('id',item.id,'version',head_version,'subject',item.subject,'identityCurrent',current_identity,'approved',approved,'pending',pending,'latest',latest,
  'canPropose',current_identity AND pending IS NULL AND ctx->'capabilities' ? 'payroll.novelty.prepare' AND ctx->>'employmentLinked'='true');
END $$;

CREATE OR REPLACE FUNCTION public.payroll_fixed_bootstrap_v1(p_context jsonb) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$
DECLARE ctx jsonb;
BEGIN
 ctx:=public.payroll_fixed_context_v1(p_context);
 RETURN jsonb_build_object('version','payroll-fixed-bootstrap.v1','principal',jsonb_build_object('tenantId',ctx->>'tenantId','membershipId',ctx->>'membershipId',
  ...
  'limits',jsonb_build_object('maxRecords',500,'maxHistory',100),'payrollTypes','["monthly","first_fortnight","sac","vacation","supplementary","final","other"]'::jsonb,'effects',public.payroll_fixed_effects_v1());
END $$;
CREATE OR REPLACE FUNCTION public.payroll_fixed_employee_v1(p_context jsonb,p_legajo text) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$
DECLARE ctx jsonb;
BEGIN
 ctx:=public.payroll_fixed_context_v1(p_context);
 RETURN jsonb_build_object('version','payroll-fixed-employee.v1','subject',public.payroll_fixed_subject_v1(ctx,p_legajo,true)->'subject');
END $$;
CREATE OR REPLACE FUNCTION public.payroll_fixed_list_v1(p_context jsonb,p_period date DEFAULT NULL) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$
DECLARE ctx jsonb; rows_json jsonb; row_count integer; fingerprint text; month_end date;
BEGIN
 ctx:=public.payroll_fixed_context_v1(p_context); PERFORM public.payroll_fixed_lock_v1(ctx);
 ...
 WITH items AS MATERIALIZED(SELECT public.payroll_fixed_record_json_v1(ctx,n) item FROM public.payroll_fixed_novelty n
 ...
 RETURN jsonb_build_object('version','payroll-fixed-list.v1','periodMonth',to_char(p_period,'YYYY-MM-DD'),'rows',rows_json,'total',row_count,'snapshotToken',fingerprint,'effects',public.payroll_fixed_effects_v1());
END $$;

CREATE OR REPLACE FUNCTION public.payroll_fixed_validate_values_v1(v jsonb) RETURNS void
LANGUAGE plpgsql IMMUTABLE SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$
...
BEGIN
 ...
 IF NOT v ?& ARRAY['conceptSourceId','costCenterSourceId','payrollType','quantityDecimal','amountCents','forced','forcedReason','legalInstrument','validFrom','validTo']
  ...
  OR NOT public.payroll_fixed_text_v1(v->>'legalInstrument',5,300)
  ...
  OR (v->>'forced'='true' AND (v->>'amountCents' IS NULL OR NOT public.payroll_fixed_text_v1(v->>'forcedReason',5,500)))
  OR (v->>'forced'='false' AND v->>'forcedReason' IS NOT NULL) THEN RAISE EXCEPTION 'PAYROLL_FIXED_INVALID_PAYLOAD'; END IF;
 date_from:=public.payroll_fixed_date_v1(v->>'validFrom'); date_to:=public.payroll_fixed_date_v1(v->>'validTo',false);
 IF date_to<date_from THEN RAISE EXCEPTION 'PAYROLL_FIXED_DATES_INVALID'; END IF;
END $$;
CREATE OR REPLACE FUNCTION public.payroll_fixed_receipt_v1(item public.payroll_fixed_novelty_event,is_duplicate boolean) RETURNS jsonb
LANGUAGE sql IMMUTABLE SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$
...
$$;
CREATE OR REPLACE FUNCTION public.payroll_fixed_event_identity_v1(ctx jsonb,item public.payroll_fixed_novelty_event) RETURNS void
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$
...
BEGIN
 ...
 PERFORM public.payroll_fixed_subject_v1(ctx,root_row.subject->>'legajo',true);
 IF NOT public.payroll_fixed_identity_current_v1(ctx,root_row) THEN RAISE EXCEPTION 'PAYROLL_FIXED_IDENTITY_CHANGED'; END IF;
END $$;
CREATE OR REPLACE FUNCTION public.payroll_fixed_attempt_v1(p_context jsonb,p_command text,p_key uuid) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$
...
BEGIN
 ...
 ctx:=public.payroll_fixed_context_v1(p_context,CASE p_command WHEN 'propose' THEN 'payroll.novelty.prepare' ELSE 'payroll.novelty.approve' END);
 PERFORM public.payroll_fixed_lock_v1(ctx);
 ...
 PERFORM public.payroll_fixed_event_identity_v1(ctx,item);
 RETURN public.payroll_fixed_receipt_v1(item,true);
END $$;
CREATE OR REPLACE FUNCTION public.payroll_fixed_propose_v1(p_context jsonb,p_payload jsonb,p_key uuid) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$
...
BEGIN
 ctx:=public.payroll_fixed_context_v1(p_context,'payroll.novelty.prepare'); PERFORM public.payroll_fixed_lock_v1(ctx);
 ...
 IF NOT p_payload ?& ARRAY['recordId','expectedVersion','contractId','legajo','identityToken','operation','values','reason']
  ...
  OR coalesce(p_payload->>'operation','') NOT IN ('set','annul') OR NOT public.payroll_fixed_text_v1(p_payload->>'reason',5,500) THEN RAISE EXCEPTION 'PAYROLL_FIXED_INVALID_PAYLOAD'; END IF;
 ...
 IF p_payload->>'operation'='set' THEN PERFORM public.payroll_fixed_validate_values_v1(p_payload->'values');
 ...
 IF FOUND THEN
  ...
  PERFORM public.payroll_fixed_event_identity_v1(ctx,previous); RETURN public.payroll_fixed_receipt_v1(previous,true);
 END IF;
 ...
 subject_value:=public.payroll_fixed_subject_v1(ctx,p_payload->>'legajo',true);
 ...
 IF root_id IS NOT NULL THEN
  ...
  view_json:=public.payroll_fixed_record_json_v1(ctx,root_row);
 ...
 END IF;
 PERFORM public.payroll_fixed_capacity_v1();
 ...
 RETURN public.payroll_fixed_receipt_v1(event_row,false);
...
CREATE OR REPLACE FUNCTION public.payroll_fixed_review_v1(p_context jsonb,p_payload jsonb,p_key uuid) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$
...
BEGIN
 ctx:=public.payroll_fixed_context_v1(p_context,'payroll.novelty.approve'); PERFORM public.payroll_fixed_lock_v1(ctx);
 IF p_key IS NULL OR p_key::text !~ '^[a-f0-9]{8}-[a-f0-9]{4}-[1-5][a-f0-9]{3}-[89ab][a-f0-9]{3}-[a-f0-9]{12}$' THEN RAISE EXCEPTION 'PAYROLL_FIXED_IDEMPOTENCY_KEY_INVALID'; END IF;
 IF p_payload IS NULL OR jsonb_typeof(p_payload)<>'object' THEN RAISE EXCEPTION 'PAYROLL_FIXED_INVALID_PAYLOAD'; END IF;
 IF NOT p_payload ?& ARRAY['recordId','proposalId','expectedVersion','decision','reason'] OR p_payload-ARRAY['recordId','proposalId','expectedVersion','decision','reason']<>'{}'::jsonb
  ...
  OR coalesce(p_payload->>'decision','') NOT IN ('approve','reject') OR NOT public.payroll_fixed_text_v1(p_payload->>'reason',5,500) THEN RAISE EXCEPTION 'PAYROLL_FIXED_INVALID_PAYLOAD'; END IF;
 ...
 PERFORM public.payroll_fixed_subject_v1(ctx,root_row.subject->>'legajo',true);
 IF NOT public.payroll_fixed_identity_current_v1(ctx,root_row) THEN RAISE EXCEPTION 'PAYROLL_FIXED_IDENTITY_CHANGED'; END IF;
 view_json:=public.payroll_fixed_record_json_v1(ctx,root_row); expected:=(p_payload->>'expectedVersion')::integer;
...
CREATE OR REPLACE FUNCTION public.payroll_fixed_export_v1(p_context jsonb,p_period date,p_snapshot text) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$
DECLARE ctx jsonb; current_list jsonb; rows_json jsonb; month_end date;
BEGIN
 ctx:=public.payroll_fixed_context_v1(p_context,'payroll.novelty.export'); PERFORM public.payroll_fixed_lock_v1(ctx);
 ...
 current_list:=public.payroll_fixed_list_v1(p_context,p_period);
 ...
 RETURN jsonb_build_object('version','payroll-fixed-export.v1','periodMonth',to_char(p_period,'YYYY-MM-DD'),'snapshotToken',p_snapshot,'rows',rows_json,'total',jsonb_array_length(rows_json),'effects',public.payroll_fixed_effects_v1());
EXCEPTION WHEN lock_not_available OR deadlock_detected THEN RAISE EXCEPTION 'PAYROLL_FIXED_SESSION_BUSY'; END $$;

CREATE OR REPLACE FUNCTION public.payroll_fixed_detail_v1(p_context jsonb,p_record uuid) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$
...
BEGIN
 ...
 IF NOT FOUND THEN RAISE EXCEPTION 'PAYROLL_FIXED_NOT_FOUND'; END IF;
 view_json:=public.payroll_fixed_record_json_v1(ctx,item);
 SELECT count(*)::integer,jsonb_agg(public.payroll_fixed_proposal_json_v1(ctx,e,(view_json->>'identityCurrent')::boolean) ORDER BY e.version DESC)
...
REVOKE ALL ON FUNCTION public.payroll_fixed_immutable_v1(),public.payroll_fixed_context_v1(jsonb,text),public.payroll_fixed_lock_v1(jsonb),public.payroll_fixed_capacity_v1(),
 public.payroll_fixed_effects_v1(),public.payroll_fixed_date_v1(text,boolean),public.payroll_fixed_text_v1(text,integer,integer),public.payroll_fixed_subject_v1(jsonb,text,boolean),
 public.payroll_fixed_identity_current_v1(jsonb,public.payroll_fixed_novelty),public.payroll_fixed_proposal_json_v1(jsonb,public.payroll_fixed_novelty_event,boolean),
 public.payroll_fixed_record_json_v1(jsonb,public.payroll_fixed_novelty),public.payroll_fixed_validate_values_v1(jsonb),public.payroll_fixed_receipt_v1(public.payroll_fixed_novelty_event,boolean),
 public.payroll_fixed_event_identity_v1(jsonb,public.payroll_fixed_novelty_event),public.payroll_fixed_bootstrap_v1(jsonb),public.payroll_fixed_employee_v1(jsonb,text),
 public.payroll_fixed_list_v1(jsonb,date),public.payroll_fixed_detail_v1(jsonb,uuid),public.payroll_fixed_attempt_v1(jsonb,text,uuid),
 public.payroll_fixed_propose_v1(jsonb,jsonb,uuid),public.payroll_fixed_review_v1(jsonb,jsonb,uuid),public.payroll_fixed_export_v1(jsonb,date,text)
 FROM PUBLIC,municontrol_actions_runtime_app;
GRANT EXECUTE ON FUNCTION public.payroll_fixed_bootstrap_v1(jsonb),public.payroll_fixed_employee_v1(jsonb,text),public.payroll_fixed_list_v1(jsonb,date),
 public.payroll_fixed_detail_v1(jsonb,uuid),public.payroll_fixed_attempt_v1(jsonb,text,uuid),public.payroll_fixed_propose_v1(jsonb,jsonb,uuid),
 public.payroll_fixed_review_v1(jsonb,jsonb,uuid),public.payroll_fixed_export_v1(jsonb,date,text) TO municontrol_actions_runtime_app;
