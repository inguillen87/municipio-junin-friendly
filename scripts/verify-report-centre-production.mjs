...
const build=publishedBuildVerification({origin,release:sha}),expected=build.expectedHashes(paths);
const end=Date.now()+240000;let ready=false;
while(Date.now()<end){try{let ok=true;for(const p of paths){const r=await build.fetchFile(p);if(!r.ok||digest(Buffer.from(await r.arrayBuffer()))!==expected[p]){ok=false;break}}if(ok){for(const [p,marker] of [['reportes-rrhh.html','assets/report-centre.js'],['nomina-control.html','assets/payroll-navigation.js'],['internal-dashboard.html','payroll-summary-status'],['novedades-nomina.html','Sin guardar; requiere validación y revisión.']]){const r=await build.fetchFile(p);if(!r.ok||!(await r.text()).includes(marker)){ok=false;break}}}if(ok){ready=true;break}}catch{}await new Promise(r=>setTimeout(r,5000))}
