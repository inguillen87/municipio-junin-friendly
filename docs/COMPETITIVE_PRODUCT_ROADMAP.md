...
|---|---|---|
| **Local** | Identidad tenant v2 ligada a sesión, MFA y ACL/capability; gobernanza 012 de propietarios globales; Actions read/write; lifecycle 009A; registro 010A; catálogo temporal 010B/011; manifiesto MariaDB→canónico; y perfilado seguro del backup municipal GRH del 19/08/2026. 010B cubre calendarios, turnos diurnos/nocturnos, reglas tipadas y asignaciones con maker-checker, idempotencia y solapamiento semanal cíclico. | Es evidencia local sobre un snapshot verdadero, no una conexión viva. La mayor parte del subsistema horario observado es histórica (2000–2012); 010A/010B/012 todavía no tienen compilación PostgreSQL QA, no están homologados para 2026 y no prueban correo real ni el artefacto desplegado. Fichadas, cálculo y posting permanecen cerrados. |
...
| Identidad, roles y auditoría | [SIGeMi](https://sigemi.com.ar/producto/) declara roles y auditoría de modificaciones; [OpenGov](https://opengov.com/) declara controles de acceso y audit trails en su plataforma. Las páginas públicas no prueban aislamiento multi-tenant ni segregación de funciones. | **Parcial avanzado**: identity gateway v2, MFA, ACL/capabilities, sesiones revocables, alta del primer administrador tenant y gobernanza maker-checker de propietarios globales implementados localmente. Persisten dos gaps de escala: asociar una identidad existente a un segundo gobierno y administrar plantillas de rol. Nada de 005–012 está desplegado en Producción. | Ejecutar QA PostgreSQL aislada, cerrar asociación multi-tenant/reactivación, desplegar Preview autorizado e invitar cuentas nominales por rol; verificar aislamiento, allow/deny, suspensión, revocación y auditoría antes de Producción. |
...
### S006 / 010A–010C — Tiempo explicable por capas — **010A/010B locales; C1–C6 planificados con fuentes reales**
...
**010B/011 implementado localmente:** el grafo horario real ya descubierto tiene manifiesto MariaDB→canónico y catálogo versionado de calendarios, turnos diurnos/nocturnos, reglas y asignaciones. Falta compilación/reapply dinámica en Neon QA y homologación funcional municipal. **010C posterior:** conectores vivos, reconciliación, simulación y ledger de excepciones; ningún cálculo o posting se adelanta.
...
2. Perfilar y homologar el backup municipal del 19/08/2026 como snapshot real: documentar `turnos`, `horarios`, `tolerancias`, `legaturn`, `esperanza`, `fichadas`, `feriado`, `prenove`, `ausencia` y `licencia`, sus relaciones, cobertura histórica, nulabilidad y vigencia. Pedir al municipio sólo las definiciones funcionales, reglas vigentes y datos de dispositivos que el dump no puede acreditar; no completar huecos con datos sintéticos.
3. Ejecutar 010B/011 exacto en la misma rama QA recién después de acreditar 010A: fresh/reapply, ACL acumulativa, nocturnidad domingo→lunes, carreras, replay, overlap y rollback; mantener cálculo y escritura GRH/payroll cerrados.
