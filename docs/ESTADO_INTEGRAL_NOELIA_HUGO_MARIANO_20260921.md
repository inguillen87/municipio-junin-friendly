...
Esta actualización sobre la base `7285166` sustituye las menciones siguientes a 101/102/103 como incrementos todavía por publicar. Conserva los cortes históricos y no declara terminados todos los módulos de Noelia, Hugo o Mariano.
...
| [103. Catálogo propio de encuadres](NOELIA_NATIVE_EMPLOYMENT_CATALOG_103.md) | Publicado en `7285166`; CI de rama `35877506021` y master `35878569712`, instalado y verificado en PG17/PG18, Vercel READY y recursos publicados cotejados. | No se acredita todavía una publicación municipal aprobada ni modifica por sí mismo el encuadre de legajos existentes. |
| [104. Rectificación de encuadres propios](NOELIA_NATIVE_EMPLOYMENT_CHANGES_104.md) | En implementación y validación: propuesta por contrato propio, comparación y decisión independiente que aplica la clasificación actual. | Sus gates de CI, instalación y publicación se acreditan separadamente. No cambia identidad, fechas, jurisdicción ni haberes; no completa baja/reingreso ni licencias nativas. |

La evidencia está conservada en los recibos privados `release-closure-405841b.private.json`, `sprint-102-close-0b8d1a8.private.json` y `catalog-release-closure.private.json`. Las verificaciones de interfaz usaron APIs interceptadas y los controles públicos incluyeron acceso anónimo: **no acreditan una sesión municipal operando legajos reales ni aceptación humana**. Esta actualización documental utiliza los recibos de cada entrega; no hace nuevas escrituras en las bases.

Brechas actuales verificadas: el alta expone creación, consulta inicial y recuperación del intento; 104 agrega un circuito separado para rectificar el encuadre. Siguen pendientes rectificaciones de identidad/fechas, baja y reingreso nativos con historia y revisión. El circuito de licencias de `lib/internal-leave-workflow.js` y sus fachadas de acciones conserva el ámbito de contratos/fuente GRH y requiere ampliación nativa. El catálogo administrativo 103 **no completa Parámetros (módulo 6)**: siguen pendientes el maestro salarial completo, escalas y fórmulas aprobadas. El motor propio de liquidación, su conciliación, cierre, integración contable y homologaciones tampoco quedan resueltos por 093/101/102/103/104.

## Ampliación de Noelia y relojes del 23/09/2026

Los módulos [9 y 10](NOELIA_MODULOS_9_10_20260923.md), el reporte de estructura de cargos de 51 páginas y el repaso completo de los manuales 1–7 quedaron incorporados a la [matriz única](MATRIZ_ACEPTACION_NOELIA.md). Se conserva también el [módulo 8](NOELIA_MODULO_8_INFORMES_20260923.md). Recibos firmados, acceso individual y control anual de cargos tienen brechas explícitas; no se marcan terminados por disponer de una biblioteca, PDF de control o encuadre de legajos.

Hugo/relojes: la ventana local de las 15:45 UTC confirma seis colectores sin bloqueo activo y nuevas capturas/acuses en La Colonia, Edificio Nuevo y PM10; para los otros equipos se conserva el último acuse y el control reciente del remitente, sin inventar nuevas marcas. PM10 conserva identidad/historia y se administra dentro del mismo parque. Oracle fue actualizado con 29 archivos cotejados y seis plantillas desactivadas. No está capturando: la VPN rechaza el certificado servidor por su uso de clave extendido, y no se migraron colas ni credenciales ni se activó un consumidor paralelo. La operación con la PC apagada continúa pendiente.

Las reglas de asistencia, cobertura, incidencias, licencias y horas se cierran mediante los casos y aprobaciones de [Hugo](ASISTENCIA_CALCULO_Y_APROBACION_HUGO_20260923.md); recibir una marcación no determina por sí mismo ausencia ni horas pagables.
