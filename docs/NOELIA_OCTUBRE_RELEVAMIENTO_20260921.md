...
El usuario entregó después las direcciones de **GAT y GAF** y se ingresó correctamente a ambas aplicaciones, una vez por aplicación. En GAT se revisaron once formularios iniciales de consulta: personas/contribuyentes, comercios, inmuebles, transferencia catastral, traslados, difuntos, deuda a apremiar, servicios públicos y gasto tributario. En GAF se verificó acceso a e-Finanzas; su relevamiento funcional sigue siendo parcial. No se ejecutaron procesos, búsquedas nominales, liquidaciones ni envíos. Ingresar a las cuatro aplicaciones no equivale a haber validado todos sus módulos.
...
| 5 Novedades | Mensuales con revisión y exportación; fijas 092 con versión y revisión independiente; ampliación nativa 093 implementada en este sprint | Validar y publicar 093 con sus gates; aceptación municipal; contratos nativos en mensuales; corrección/anulación masiva 5.5; consumo por el motor salarial |
...
1. **Alta nativa y consumidores.** 067 crea contratos `MUNICONTROL`. La ampliación 093 implementa alta propia → novedad fija → revisión independiente → consulta/exportación mediante el UUID exacto y su registro de origen, preservando los sujetos GRH previos. 026 (mensuales) y 064 → 091 (familia/escolaridad) siguen pendientes de esa extensión. La aceptación operativa con usuarios municipales se verifica por separado.
2. **SQL histórico reproducible.** Se recuperó 041 desde el commit original `9a91a56453a91ef4a8deba72c2b4f853545de8a2`, sin ejecutarla. Sus catorce cuerpos de funciones coinciden con PG17 y PG18, normalizando saltos de línea. Se conserva la referencia versionada para ampliar 066 y la API de parámetros; todavía falta certificar el resto del esquema y las reglas del dominio.
