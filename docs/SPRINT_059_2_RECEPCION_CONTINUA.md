...
Se agrega recepción de partes del colector PM-10, conservación inmutable, acuses recuperables y una vista autenticada de recepción. El lector físico v4.1 y el servicio de captura 059.1.1 no se modifican. No hay instalación municipal en esta entrega ni prueba de una nueva fichada física. La publicación del receptor no activa el conector.

## Circuito
Captura local completa → cola existente → remitente HTTPS separado → validación/transacción PostgreSQL → acuse persistente local → consulta del panel. Cada parte transporta hasta 500 registros originales de 40 bytes, ordinales, fecha de captura declarada y huellas. El límite por solicitud es 64 KiB. El servidor verifica bytes, tamaño, huella de la parte y consistencia de la cabecera; las huellas de la descarga completa y del delta identifican la procedencia declarada, pero no certifican por sí solas cobertura del período ni reconstrucción de la descarga completa. No se presentan como firma digital.
...
## Interfaz
`Relojes y marcaciones → Recepción del colector municipal`: nuevos eventos, registros reconocidos, observaciones, partes confirmadas, última captura declarada y última recepción. Lista hasta 50 registros nuevos, con nombre/legajo sólo con permiso nominal. La captura histórica, sus exportaciones y las jornadas existentes siguen separadas; no se mezclan silenciosamente.
