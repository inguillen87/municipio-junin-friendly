...
El manual operativo accesible de GRH_WEB confirma que “Recibo de sueldo” se consulta por legajo, fecha, mes, año y tipo de liquidación. La tabla oficial de `GRHW_Formulas_Acrobat.pdf` (página física 28, folio 16) vincula expresamente los seis códigos: `P` primera quincena, `M` mensual o segunda quincena, `F` liquidación final, `V` vacaciones anuales, `S` sueldo anual complementario y `O` otras liquidaciones. La página física 63, folio 51, vuelve a documentar `P/M/S/F/O`.

El 04/09/2026 se contrastó esa tabla, sin ejecutar el informe, contra el selector vivo `process.tipo31` de `GRH > Nómina > Informes > Recibo de Sueldo`: `F=Liquidación Final`, `M=Mes`, `O=Otros Conceptos`, `P=Primera Quincena`, `S=SAC` y `V=Vacaciones`. El respaldo `grh_junin.backup_2026081915_plataforma.sql.gz` sólo contiene esos seis valores no vacíos en `calculo`, `histocal` y `legamov`; seis filas vacías de `legamov` permanecen como anomalía de calidad, no como otro tipo.
...
“Cargar novedad” transfiere legajo, período y, sólo cuando ya es canónico, tipo mediante `sessionStorage` de un solo uso y cinco minutos de vigencia. La presentación muestra primero el nombre cotidiano —por ejemplo, “Aguinaldo (SAC)”— y conserva el código GRH como trazabilidad secundaria. No coloca datos personales en la URL, no prepara un lote y no lo envía automáticamente. El formulario conserva la validación humana y el circuito maker-checker existentes.

El contrato v1 y la migración 032 conservan la primera evidencia (`M → monthly`) como ledger inmutable. La evidencia oficial posterior permite una versión nueva con `P → first_fortnight`, `S → sac`, `V → vacation`, `O → other` y `F → final`. `O` no se interpreta como suplementaria: la fuente sólo prueba “otras liquidaciones”. Cualquier código distinto de los seis observados, incluido el vacío, continúa sin clasificar y bloquea una decisión que dependa de su significado.
...
Con C1 y D0 ya implementados únicamente como control y descarga locales, el orden inmediato para convertir ese resultado en una operación institucional gobernada es **S008-B2 → S008-C2 → S008-D**. La base B2.1 ya define en código el parser servidor y la migración versionada para recibir el corte en cuarentena con HMAC contextual, procedencia `operator_declared`, idempotencia y separación entre preparador y validador. Todavía no está aplicada a Neon ni conectada a una API o pantalla operativa. C2 conservará la conciliación general, las diferencias justificadas y el maker-checker en una corrida reproducible; D generará el Excel y el PDF institucionales desde esa misma corrida aprobada. C1, D0 y B2.1 no reemplazan ninguno de esos gates operativos.
...
- **S008-B2.1 implementado en código, aún no operativo:** existe el contrato privado tenant-bound para los dos agregados 701/703, con importes `BIGINT`, fuente declarada, cuarentena, auditoría append-only, idempotencia, versionado optimista y capacidades separadas para preparación, validación y consulta. Los perfiles previstos son Marcelo/preparación y Hugo/validación, pero sus membresías efectivas todavía deben verificarse en el entorno objetivo. El aplicador sólo admite una rama Neon aislada, directa y fijada por proyecto, branch, host y base; no se ejecutó contra Neon ni Producción;
- **S008-B2 operativo, S008-C2 y S008-D homologado permanecen pendientes:** faltan la API dedicada que use el parser y las fachadas, la aplicación/aceptación en una rama aislada, la pantalla de bandeja y aprobación, la corrida persistida, la observación durable y la generación institucional desde una corrida aprobada. Por eso los archivos D0 no deben presentarse como cierre o documento institucional aprobado.
