import {LEGAL_ALERT_CATEGORIES,LEGAL_ALERT_SOURCES,filterLegalAlerts,legalAlertCategory,legalAlertCounts,verifyLegalAlertCenterResponse} from './legal-alert-center-model.js';
...
async function start(host){
 ...
 async function load(){busy=true;render();try{data=await request();message='Alertas actualizadas con fechas revisadas por el municipio.';error=false;}catch(e){if([401,403].includes(e.status)){data=null;blocked=true;message='El acceso cambió. Las alertas jurídicas se ocultaron.';error=true;}else{message=e.message;error=true;}}finally{busy=false;render();}}
 ...
 function render(){
  ...
  if(blocked)return;if(!data){if(!busy)button(host,'Reintentar',load);return;}
  const counts=legalAlertCounts(data.rows,data.today),cats=add(host,'div','','lac-categories');
  ...
  for(const r of visible){const c=add(cards,'article','','lac-card');add(c,'span',LEGAL_ALERT_CATEGORIES[legalAlertCategory(r,data.today)],'lac-tag');add(c,'h2',r.title);add(c,'p',resourceLabel(r)+' · '+r.sourceKind+' '+r.sourceNumber+'/'+r.sourceYear+' · '+r.sourceTitle);add(c,'p','Fecha registrada: '+(r.dueDate||'Sin fecha')+(r.responsibleLabel?' · Responsable: '+r.responsibleLabel:''),'lac-note');add(c,'p','Estado registrado: '+r.status,'lac-note');const t=add(c,'div','','lac-tools');const a=add(t,'a','Abrir recurso','button');a.href=resourceHref(r);}
 ...
 const access=await globalThis.MuniControlCapabilityGate?.ready;if(!access?.tenantCapabilities?.has('legal.norm.read')){blocked=true;message='No tenés acceso a alertas jurídicas.';error=true;render();return;}await load();
}
