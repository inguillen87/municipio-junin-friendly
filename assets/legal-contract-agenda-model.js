...
export function verifyContractAgendaResponse(d){
 ...
 if(d.version!=='legal-contract-agenda.v1'||!/^\d{4}-\d{2}-\d{2}$/.test(d.today)||d.timezone!=='America/Argentina/Mendoza'||d.limit!==1000||!integer(d.population,0,1000)||!/^[a-f0-9]{64}$/.test(d.revision||'')||!Array.isArray(d.rows)||d.rows.length!==d.population)fail();
 const ids=new Set();
