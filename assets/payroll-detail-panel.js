...
export async function openPayrollDetail({host,employee,item,request,canRead,onClose}){
 ...
 const close=(restoreFocus=false)=>{if(!active)return;active=false;controller.abort();observer.disconnect();section.remove();if(restoreFocus&&canRead())onClose?.()};
 header.append(title,button('Cerrar detalle',()=>close(true)));section.append(header,status);host.append(section);
 ...
 try{
  ...
  }catch(e){if(active&&section.isConnected&&canRead()&&e.name!=='AbortError')feedback.textContent=e.message||'No se generó el archivo.'}finally{all.forEach(x=>x.disabled=!active||!section.isConnected||!canRead())}}
 ...
 }catch(e){if(active&&section.isConnected&&canRead()&&e.name!=='AbortError'){status.textContent=e.message||'No se pudo consultar el detalle.';status.classList.add('pd-warning')}}
}
