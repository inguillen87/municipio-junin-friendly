...
import { FIXED_TYPES,fixedCoverage,fixedMoney,fixedOriginLabel } from './payroll-fixed-novelties-model.js';

const headings=['Período de consulta','Legajo','Agente','Concepto declarado','Centro de costo','Tipo de liquidación','Unidades completas','Importe completo ARS','Importe exacto centavos','Modo forzado','Fundamento forzado','Instrumento','Alta','Vencimiento','Alcance de vigencia','Corte de identidad','Revisión del registro','Origen del vínculo','Fecha del alta propia'];
function report(snapshot,options={}){
  ...
  const rows=snapshot.rows.map(r=>{const v=r.values,s=r.subject;if(!v||!s||!fixedCoverage(v,snapshot.periodMonth).intersects)throw Error('La vigencia no corresponde al período de exportación.');
    return [snapshot.periodMonth,s.legajo,s.employeeName??'Nombre no informado',v.conceptSourceId,v.costCenterSourceId??'Sin informar',FIXED_TYPES[v.payrollType],v.quantityDecimal??'Sin informar',fixedMoney(v.amountCents),v.amountCents??'Sin informar',v.forced?'Sí':'No',v.forcedReason??'No corresponde',v.legalInstrument,v.validFrom,v.validTo??'Sin vencimiento informado',fixedCoverage(v,snapshot.periodMonth).label,s.sourceCutoff??'No corresponde · alta propia',String(r.version),fixedOriginLabel(s),s.registeredAt??'No corresponde'];});
