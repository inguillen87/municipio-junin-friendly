...
async function request(resource,body,key){
 const options={method:body?'POST':'GET',credentials:'same-origin',cache:'no-store',headers:{Accept:'application/json'}};
...
const formMarkup=`<header class="ne-heading"><p>PERSONAS · ALTA PROPIA</p><h2 id="ne-title">Nuevo legajo municipal</h2><span>Registrá a la persona y su encuadre laboral en MuniControl, sin importar un archivo de GRH.</span></header>
 ...
 <label>Repartición / organización<select name="organizationId" required><option value="">Seleccioná repartición</option></select></label>
 <label>Sector<select name="sectorCode" required><option value="">Seleccioná sector</option></select></label>
...
export function mountEmployeeCreate(button){
 ...
 function selectOptions(name,items,hint){const select=form.elements[name];select.replaceChildren(new Option(hint,''));for(const item of items)select.add(new Option(item.code+' · '+item.label,item.code));}
 ...
 async function initialize(){
  ...
  try{const value=await request({resource:'bootstrap'});if(token!==generation)return;
   ...
   context=value;for(const [name,kind,hint]of [['agreementCode','agreements','Seleccioná convenio'],['organizationId','organizations','Seleccioná repartición'],['sectorCode','sectors','Seleccioná sector']])selectOptions(name,catalog(kind),hint);
 ...
 form.addEventListener('submit',e=>{e.preventDefault();if(busy||blocked||!context?.canCreate)return;
  try{const draft=employeeDraft(Object.fromEntries(EMPLOYEE_FIELDS.map(k=>[k,form.elements[k].value])),context.today);prepared={draft,catalogVersion:context.catalog.version};
   const rows=[['Persona',draft.fullName],['DNI / CUIL',draft.dni+' / '+draft.cuil],['Legajo',draft.legajo||'Asignación automática'],['Ingreso',draft.startDate],['Convenio',form.elements.agreementCode.selectedOptions[0].text],['Categoría',form.elements.categoryCode.selectedOptions[0].text],['Repartición',form.elements.organizationId.selectedOptions[0].text],['Sector',form.elements.sectorCode.selectedOptions[0].text],['Documento de alta',draft.legalReference]];
