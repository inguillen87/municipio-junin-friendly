...
export function mountNoveltySheet(host, { onChange = () => {}, pickEmployees = null, maximumRows = () => 500 } = {}) {
  ...
  const find = button('Buscar legajos activos','sheetFind'); find.hidden = !pickEmployees;
  toolbar.append(add,group,find,restore,clear); const state=el('p','','sheet-state');state.id='sheetState';state.setAttribute('role','status');state.setAttribute('aria-live','polite');
  ...
  function render(focusIndex=null){
    ...
    for(let i=0;i<view.rows.length;i++){
      ...
      for(const [column,name,max]of [[0,'Legajo',20],[1,'Concepto',20],[4,'Unidades',24]]){
        const td=el('td');td.dataset.label=name;const field=input(index,column,name,max);td.append(field);
        if(column===0 && pickEmployees){
          const fieldWrap=el('div',undefined,'picker-sheet-field'), lookup=button('Buscar por nombre',null,'compact'),chosen=el('small',pickedNames.get(row[0])||'');
          lookup.setAttribute('aria-label','Buscar legajo de la fila '+(index+1));lookup.disabled=locked;fieldWrap.append(field,lookup,chosen);td.append(fieldWrap);
          field.addEventListener('input',()=>{chosen.textContent=pickedNames.get(field.value)||'';});
          lookup.addEventListener('click',()=>{const target=rows[index];pickEmployees({onUse:items=>{
            if(locked||rows[index]!==target)throw Error('La fila cambió. Volvé a consultar.');
            pickedNames.set(items[0].legajo,items[0].nombre||'Nombre no informado');
            update(index,0,items[0].legajo);field.value=items[0].legajo;chosen.textContent=pickedNames.get(items[0].legajo);
          }});});
        }
        tr.append(td);
      }
  ...
  $('sheetGroupFind').hidden=!pickEmployees;
  $('sheetGroupFind').addEventListener('click',()=>{
    if(locked||!pickEmployees)return;
    try{
    ...
    }catch(error){$('sheetGroupError').textContent=error.message;$('sheetGroupError').hidden=false;}
  });
