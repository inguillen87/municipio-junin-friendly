...
if(root){
 ...
 function clear(){for(const k of ['new','known','observed','parts','received','captured'])$(k).textContent='—';$('rows').replaceChildren();}
 function cell(tr,v){const td=document.createElement('td');td.textContent=v??'—';tr.append(td);}
 function valid(x){return x?.ok===true&&x.version==='pm10-status.v1'&&x.physicalClockVerified===false&&x.payrollModified===false&&['active','suspended','retired','not_configured'].includes(x.connectorState)&&Number.isSafeInteger(x.baselineRecords)&&x.baselineRecords>=0&&typeof x.nominalReadAllowed==='boolean'&&Array.isArray(x.records)&&x.records.length<=50&&['receipts','newMarks','knownRecords','observations'].every(k=>Number.isSafeInteger(x.summary?.[k])&&x.summary[k]>=0)&&x.records.every(r=>typeof r.personLabel==='string'&&['observed','mapped','review'].includes(r.state)&&(x.nominalReadAllowed||(r.legajo===null&&r.personLabel==='Identidad reservada')));}
 function render(x){
