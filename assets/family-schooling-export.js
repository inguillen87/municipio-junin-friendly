...
export function schoolingXlsx(data, view, queriedAt) {
  if (data?.version !== 'family-schooling.v1' || !Array.isArray(view?.rows) || view.rows.length > MAX_SCHOOLING_ROWS
  ...
  const headers = ['Legajo', 'Agente', 'Hijo/a', 'Nacimiento informado', 'Baja del vínculo familiar', 'Estado del legajo al corte',
    'Presentación registrada', 'Vencimiento registrado', 'Registro en MuniControl', 'Archivo', 'Control documental', 'Corte de origen'];
  ...
  const control = [['Control y procedencia', 'Valor'], ['Reporte', 'Legajos activos con hijos · certificados escolares'],
    ['Uso', 'Control interno. No aprueba escolaridad ni habilita haberes.'], ['Fuente de los vínculos', 'GRH incorporado en MuniControl'],
    ...
    ['Hijos/as del filtro', view.counts.children], ['Con certificado registrado', view.counts.registered],
  ...
  return storedZip([
    ...
    ['xl/styles.xml', styles], ['xl/worksheets/sheet1.xml', sheet(rows, [15, 30, 30, 20, 22, 22, 22, 24, 29, 35, 35, 20], true)],
