...
import { certificateState, schoolingEffectiveDates, schoolingDateOrigin, MAX_SCHOOLING_ROWS } from './family-schooling-model.js';
...
export function schoolingXlsx(data, view, queriedAt) {
  if (!['family-schooling.v1','family-schooling.v2','family-schooling.v3','family-schooling.v4'].includes(data?.version) || !Array.isArray(view?.rows) || view.rows.length > MAX_SCHOOLING_ROWS
  ...
  const unified = data.version !== 'family-schooling.v1', complete = ['family-schooling.v3','family-schooling.v4'].includes(data.version), sourceDates = data.version === 'family-schooling.v4';
  ...
  return storedZip([
    ...
    ['xl/styles.xml', styles], ['xl/worksheets/sheet1.xml', sheet(rows, [15, 30, 30, 20, 22, 22, 22, 24, 29, 35, 35, 20, ...(unified ? [20,32,28,24,42] : []), ...(complete ? [35,24,24,22,22,36,45,45,22] : []), ...(sourceDates ? [38,24,24,36,36,30,32,44] : [])], true)],
