...
import { MONTHLY_SUMMARY_VERSION, monthlyDecimal, monthlyClosure, monthlyType, monthlyObservations, MUTUAL_RETENTIONS_PRESET, checkMutualRetentions, mutualDecimal } from './payroll-monthly-summary-model.js';
...
function controlRows(data, view, queriedAt, retentions = null) {
  const rows = [['Control y procedencia', 'Valor'], ['Reporte', 'Resumen mensual · corridas seleccionadas'], ['Período de imputación', data.period],
  ...
  if (!retentions) return rows;
  const result = rows.filter(row => !['Búsqueda', 'Filtro', 'Cantidades', 'Totales', 'Conceptos exportados'].includes(row[0]));
  result.find(row => row[0] === 'Reporte')[1] = 'Detalle de lo retenido a mutuales · Control de selección';
  result.push(['Referencia de preconfiguración', MUTUAL_RETENTIONS_PRESET.source],
    ['Versión de preconfiguración', MUTUAL_RETENTIONS_PRESET.id], ['Selección', retentions.presetUnchanged ? 'Coincide con los códigos de la muestra; no certifica su vigencia en otro período.' : 'Selección editada. La muestra no determina automáticamente su alcance.'],
    ['Conceptos seleccionados', String(retentions.codes.length)], ['Conceptos ausentes', String(retentions.absentCount)],
    ['Conceptos con importe faltante', String(retentions.missingCount)], ['Conceptos no sumables', String(retentions.unverifiedCount)],
    ['Regla de suma', 'Sólo componentes con totalización 996, sin totalizadores ni códigos repetidos. No se recalculan retenciones ni porcentajes.'],
    ['Subtotal informado', mutualDecimal(retentions.subtotal) + ' · No representa el total cuando hay pendientes.'],
    ['Total de la selección', mutualDecimal(retentions.total)], ['Firma y pago', 'Sin firma aplicada. No es una nota emitida ni acredita una transferencia.']);
  for (let i = 0; i < retentions.codes.length; i += 20) result.push(['Códigos seleccionados · ' + (i / 20 + 1), retentions.codes.slice(i, i + 20).join(', ')]);
  return result;
}
function sheet(rows, widths, filter = false) {
  const body = rows.map((row, i) => {
    const lines = Math.max(...row.map((value, j) => monthlyPdfLines(value?.numeric ?? value ?? '', Math.max(20, widths[j] * 5.25 - 12), 11, i === 0).length));
    return `<row r="${i + 1}" ht="${Math.min(409, Math.max(36, lines * 14 + 12))}" customHeight="1">` + row.map((v, j) => {
...
export function monthlySummaryXlsx(data, view, queriedAt, retentions = null) {
  checked(data, view, queriedAt);
  if (retentions) checkMutualRetentions(data, retentions);
  const concepts = retentions ? [['Código de descuento', 'Concepto en la fuente', 'Importe retenido', 'Estado / observaciones'],
    ...retentions.rows.map(r => [r.code, r.description, mutualExcelDecimal(r.amount), r.observation]),
    ['', 'Subtotal de importes informados', mutualExcelDecimal(retentions.subtotal), 'Sólo componentes verificables con importe informado. No reemplaza el total pendiente.'],
    ['', 'Total de la selección', mutualExcelDecimal(retentions.total), retentions.total === null ? 'Hay conceptos ausentes, con importe faltante o no sumables.' : 'Todos los conceptos seleccionados tienen importe informado y son sumables.']]
    : [headings, ...view.rows.map(r => [r.code, r.description, r.unit ?? 'No informada', number(r.sourceRows), number(r.distinctLegajos),
  ...
  const names = [retentions ? 'Retenciones' : 'Conceptos', 'Fuentes', 'Control'];
  ...
  return storedZip([
    ...
    ['xl/styles.xml', styles], ['xl/worksheets/sheet1.xml', sheet(concepts, retentions ? [18, 52, 32, 88] : [12, 43, 16, 16, 17, 28, 32, 48], !retentions)],
    ['xl/worksheets/sheet2.xml', sheet(sources, [40, 38, 20, 20, 35, 24, 20, 18, 70, 70, 34], true)],
    ['xl/worksheets/sheet3.xml', sheet(controlRows(data, view, queriedAt, retentions), [36, 110])],
  ]);
}
...
export function monthlySummaryPdf(data, view, queriedAt, retentions = null) {
  checked(data, view, queriedAt);
  if (retentions) checkMutualRetentions(data, retentions);
  ...
  function page() {
    ...
    draw(32, 26, retentions ? 'MuniControl | Retenciones a mutuales' : 'MuniControl | Resumen mensual', 17, true, '1 1 1');
  ...
  function table(title, labels, values, widths, continuePage = false) {
    ...
    const required = 36 + Math.max(...labels.map((value, index) => monthlyPdfLines(value, sizes[index] - 12, 8, true).length)) * 10 + 14
      + values.reduce((height, row) => height + Math.max(...row.map((value, index) => monthlyPdfLines(value, sizes[index] - 12).length)) * 11 + 14, 0);
    if (!continuePage || !ops || y + required > 546) page();
    else { y += 18; draw(32, y + 9, title, 10, true); y += 18; }
    header();
  ...
  if (retentions) {
    table('Detalle de la selección', ['Código', 'Concepto en la fuente', 'Importe retenido', 'Estado / observaciones'],
      retentions.rows.map(r => [r.code, r.description, mutualDecimal(r.amount), r.observation]), [12, 40, 30, 65]);
    table('Totales de la selección', ['Control', 'Importe', 'Alcance'], [
      ['Subtotal informado', mutualDecimal(retentions.subtotal), 'Sólo conceptos sumables con importe informado; no sustituye el total pendiente.'],
      ['Total de la selección', mutualDecimal(retentions.total), retentions.total === null ? 'Hay conceptos ausentes, con importe faltante o no sumables.' : 'Todos los conceptos seleccionados tienen importe informado y son sumables.']], [35, 30, 65], true);
  } else table('Conceptos del filtro', headings, view.rows.map(r => [r.code, r.description, r.unit ?? 'No informada', String(r.sourceRows), String(r.distinctLegajos), monthlyDecimal(r.quantity), monthlyDecimal(r.amount), monthlyObservations(r)]), [10, 29, 13, 13, 14, 23, 27, 27]);
  ...
  const controls = controlRows(data, view, queriedAt, retentions); table('Control y alcance', controls[0], controls.slice(1), [30, 100]);
...
function mutualExcelDecimal(raw) {
  if (raw === null) return 'No determinable';
  mutualDecimal(raw);
  return raw.replace(/[-.]/g, '').replace(/^0+/, '').length <= 15 ? number(raw) : raw;
}
function mutualExportView(data, report) {
  checkMutualRetentions(data, report);
  const codes = new Set(report.codes);
  return { rows: data.rows.filter(r => codes.has(r.code)), filters: { search: '', status: 'all' } };
}
export function mutualRetentionsXlsx(data, report, queriedAt) {
  return monthlySummaryXlsx(data, mutualExportView(data, report), queriedAt, report);
}
export function mutualRetentionsPdf(data, report, queriedAt) {
  return monthlySummaryPdf(data, mutualExportView(data, report), queriedAt, report);
}
