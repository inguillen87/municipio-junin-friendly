...
export function mountBackupReview(host) {
  ...
  host.innerHTML = `<header class="section-head"><div><span class="section-index">REVISIÓN LOCAL · SIN INCORPORACIÓN</span><h2>Revisar un respaldo antes de incorporarlo</h2><p>Abrí el informe agregado generado al comparar dos respaldos en el equipo local. El respaldo SQL permanece fuera de esta pantalla.</p></div></header>
    ...
    <small id="backupReviewHelp">Podés abrir las revisiones de respaldo, núcleo salarial, quince archivos de personal o el informe coordinado. No selecciones el respaldo SQL ni archivos con datos personales.</small>
  ...
  function clearResult() { result.hidden = true; const curated = $('[data-br-curated]'); if(curated){curated.hidden=true;curated.replaceChildren();} const extra = $('[data-br-successor]'); if (extra) { extra.hidden = true; extra.replaceChildren(); } $('[data-br-domains]').replaceChildren(); $('[data-br-issues]').replaceChildren(); $('[data-br-trace]').replaceChildren(); $('[data-br-core-corrections]').textContent = ''; $('[data-br-core-corrections]').hidden = true; }
